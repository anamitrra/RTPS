...
<?php 
$applications = !empty($acmrnoc) ? $acmrnoc : array()
?>
<div class="container">
    <div class="row">
        <div class="col-sm-12 mx-auto">
            <div class="card my-4">
                <div class="card-body table-container">
                  <h4>Issuance of No Objection Certificate -ACMR </h4>
                  <?php if (!empty($applications)): ?>
                        ...
                        <table class="table">
                          ...
                          <tbody id="">
                            ...
                            <tr>
                              <td><?=($key+1)?></td>
                              <td><a target="_blank" href="<?=base_url("spservices/acmrnoc/noc/applicationpreview/").$obj_id ?>" ><?= $value->service_data->appl_ref_no ?></a></td>
                              ...
                              <td>
                                  <?php if($value->service_data->appl_status == "DRAFT") { ?>
                                    <a class="btn btn-primary btn-sm mbtn" href="<?=base_url("spservices/acmrnoc/noc/index/").$obj_id ?>" >Complete Your Application</a>
                                  <?php }
                                  ...
                                  <?php } elseif($value->service_data->appl_status == "QS"){
                                    ?>
                                    <a class="btn btn-success btn-sm mbtn" href="<?=base_url("spservices/applications/acknowledgement/").$obj_id ?>" >Acknowledgement</a>
                                    <a class="btn btn-warning btn-sm mbtn" href="<?= base_url('spservices/acmrnoc/noc/queryform/'.$obj_id)?>" >Reply to query</a>
                                    <?php 
                                          ...
                                          <?php if(isset($value->form_data->pfc_payment_response) && !empty($value->form_data->pfc_payment_response->GRN) && ($value->form_data->pfc_payment_response->STATUS === "P" || $value->form_data->pfc_payment_response->STATUS === "")){
                                              ?>
                                               <form method="post" name="getGRN" id="getGRN" action="<?=$this->config->item('egras_grn_cin_url')?>" >
                                                    ...
                                                    <input type="hidden" id ="ACTION_CODE" name="ACTION_CODE" value="GETCIN" readonly/>
                                                    <input type="hidden" id ="SUB_SYSTEM" name="SUB_SYSTEM" value="ARTPS-SP|<?=base_url('spservices/acmrnoc/payment_response/cin_response')?>" />
                                            ...
                                            ?>
                                            <a class="btn btn-warning btn-sm mbtn" href="<?= base_url('spservices/acmrnoc/noc/submit_to_backend/'.$obj_id)?>" >Verify Payment</a>
                                            <?php
                                          }else{  ?>
                                              <a class="btn btn-warning btn-sm mbtn" href="<?= base_url('spservices/acmrnoc/payment/verify/'.$obj_id)?>" >Make/Verify Payment</a>
                                          <?php }
                                          ?>
                                  ...
                                  ?>
                                   ...
                                   <a href="<?=base_url('spservices/acmrnoc/noc/track/'.$obj_id)?>" class="btn btn-secondary btn-sm mbtn" >Track</a>
                              </td>
