...
<?php 
$applications = !empty($castecertificate) ? $castecertificate : array();
                  ...
                  <?php if (!empty($applications)): ?>
                        ...
                        <table class="table">
                          ...
                          <tbody id="">
                            ...
                            <tr>
                              <td><?=($key+1)?></td>
                              <td><a href="<?=base_url("spservices/castecertificate/registration/applicationpreview/").$obj_id ?>" target="_blank"><?= $value->service_data->appl_ref_no ?></a></td>
                              ...
                              <td>
                                  <?php if($value->service_data->appl_status == "DRAFT") { ?>
                                    <a class="btn btn-primary btn-sm mbtn" href="<?=base_url("spservices/castecertificate/registration/index/").$obj_id ?>" >Complete Your Application</a>
                                  <?php }
                                  ...
                                  <?php } elseif($value->service_data->appl_status == "QS"){
                                    ?>
                                    <a class="btn btn-success btn-sm mbtn" href="<?=base_url("spservices/applications/acknowledgement/").$obj_id ?>" >Acknowledgement</a>
                                    <a class="btn btn-warning btn-sm mbtn" href="<?= base_url('spservices/castecertificate/registration/queryform/'.$obj_id)?>" >Reply to query</a>
                                    <?php 
                                          ...
                                          <?php if(isset($value->form_data->pfc_payment_response) && !empty($value->form_data->pfc_payment_response->GRN) && ($value->form_data->pfc_payment_response->STATUS === "P" || $value->form_data->pfc_payment_response->STATUS === "")){
                                              ?>
                                               <form method="post" name="getGRN" id="getGRN" action="<?=$this->config->item('egras_grn_cin_url')?>" >
                                                    ...
                                                    <input type="hidden" id ="ACTION_CODE" name="ACTION_CODE" value="GETCIN" readonly/>
                                                    <input type="hidden" id ="SUB_SYSTEM" name="SUB_SYSTEM" value="ARTPS-SP|<?=base_url('spservices/castecertificate/payment_response/cin_response')?>" />
                                                    <input type="submit" style="margin-top: 3px;color:white" id ="submit" class="btn btn-sm btn-warning mbtn" name="submit" target = "_BLANK" value="Verify Payment" />
                                                  </form>
                                              <?php
                                            ...
                                            ?>
                                            <a class="btn btn-warning btn-sm mbtn" href="<?= base_url('spservices/castecertificate/registration/finalsubmition/'.$obj_id)?>" >Verify Payment</a>
                                            <?php
                                          }else{  ?>
                                              <a class="btn btn-warning btn-sm mbtn" href="<?= base_url('spservices/castecertificate/payment/verify/'.$obj_id)?>" >Make/Verify Payment</a>
                                          <?php }
                                          ?>
                                  ...
                                  <?php }
                                  ?>
                                 <a href="<?=base_url('spservices/castecertificate/registration/track/'.$obj_id)?>" class="btn btn-secondary btn-sm mbtn" >Track</a>
                                   <a href="<?=base_url('spservices/castecertificate/registration/track/'.$obj_id)?>" class="btn btn-secondary btn-sm mbtn" ><i class="fa fa-refresh"></i> Refresh Status</a>
                              </td>
