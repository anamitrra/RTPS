        ...
        <div class="card shadow">
            ...
            <div class="card-body">
                ...
                <?php if (isset($dbrow->service_data->appl_status) && $dbrow->service_data->appl_status == "QS") { ?>
                    <p style="font-size:18px;"><label><i class="fa fa-hand-o-right" aria-hidden="true"></i><span class="text-danger"> Waiting for applicant response. Please <a href="<?= base_url('spservices/kaacincomecertificate/registration/queryform/' . $dbrow->_id->{'$id'}) ?>" class="btn btn-info btn-sm">click here</a> to proceed.</span></label></p>
                <?php } elseif (isset($dbrow->service_data->appl_status) && $dbrow->service_data->appl_status == "D") { ?>
                    <a class="btn btn-success btn-sm mbtn text-center" target="_blank" href="<?= base_url('spservices/kaacincomecertificate/registration/download_certificate/' . $dbrow->_id->{'$id'}) ?>"><i class="fa fa-download" aria-hidden="true"></i> DOWNLOAD CERTIFICATE</a>
                <?php }
                ?>
