<?php
$obj_id = $dbrow->_id->{'$id'};
$full_name = isset($dbrow->form_data->applicant_name) ? $dbrow->form_data->applicant_name : '';
$state = "Assam";
...
// $unformat_date = format_mongo_date($dbrow->service_data->created_at);
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on("click", "#finalBtn", function() {
            location.replace("<?= base_url('spservices/employmentnonaadhaar/reregistration/submit_query/' . $obj_id) ?>")
        });
    });
</script>
...
<main class="rtps-container">
    <div class="container my-2">
        <div class="card shadow-sm" id="printDiv">
            <div class="card-header" style="background:#589DBF;text-align: center; font-size: 20px; color: #fff; font-family: georgia,serif; font-weight: bold">
...
<!-- 
                ...
                <fieldset class="border border-success table-responsive" style="margin-top:10px">
                    ...
                    <table class="table table-borderless" style="margin-top:0px; margin-bottom:0px;">
                        <tbody style="border-top: none !important">
                            <tr>
                                <td style="width:50%">Name of the Applicant<strong> : <?= $full_name ?></strong> </td>
                ...
                <fieldset class="border border-success table-responsive" style="margin-top:10px">
                    <legend class="h5">Time Slot for Physical Document Verification</legend>
                    <table class="table table-borderless" style="margin-top:0px; margin-bottom:0px;">
                        <tbody style="border-top: none !important">
                            ...
                            <tr>
                                <td style="width:50%">Date<strong> : <?= $dbrow->form_data->booking_date ?> </strong> </td>
                                <td style="width:50%">Time Slot<strong> : <?= $dbrow->form_data->time_slot ?> </strong> </td>
                            </tr>
                        </tbody>
                    </table>
                </fieldset>
            ...
            <div class="card-footer text-center no-print">
                ...
                <?php 
                if ($status == 'QS') { ?>
                    <button class="btn btn-success" id="finalBtn" type="button">
                        <i class="fa fa-check"></i> Final Submit
                    </button>
                    <a class="btn btn-primary frmbtn" id="DRAFT" href="<?= base_url('spservices/employment-reregistration-nonaadhaar/enclosures/' . $obj_id) ?>">
                ...
                <?php } else { ?>
                    <?php if ($payment_status !== 'Y') { ?>
                        ...
                        <?php if ($user_type == 'user') { ?>
                            <a href="<?= base_url('spservices/employmentnonaadhaar/Payment/initiate/' . $obj_id) ?>" class="btn btn-warning">
