...
class Application_model extends Mongo_model
{
    ...
    {
        $collection = 'minoritycertificates';
        $operations = [
            [
                '$match' => [
    ...
    {
        $collection = 'minoritycertificates';
        if ($this->session->userdata('mcc_user')['role_slug'] == 'DPS') {
            $match = [
                'service_id' => $service_id,
                'pa_district_name' => $this->session->userdata('mcc_user')['district_name'],
                'status' => ['$in' => ["PAYMENT_COMPLETED", "UNDER_PROCESSING", "QUERY_ARISE", "QUERY_SUBMITTED"]],
                '$or' => [
                    ['execution_data.0.task_details.user_detail.user_id' => $this->session->userdata('mcc_user')['userId']->{'$id'}],
    ...
    //     $applications = $this->mongo_db->get('minoritycertificates');
    //     return $applications;
    // }
