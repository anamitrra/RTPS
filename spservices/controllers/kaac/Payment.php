...
class Payment extends Rtps
{
    ...
    {
        $service_ids = array("DCTH", "CCJ", "CCM", "DLP", "ITMKA", "LHOLD", "LRCC", "NECKA", "LVC");
        if (in_array($service_id, $service_ids)) {
            $ref = modules::load('spservices/kaac/registration');
    ...
    private function check_pfc_payment_status($DEPARTMENT_ID = null)
    {
        if ($DEPARTMENT_ID) {
            ...
            $ref = modules::load('spservices/kaac/Payment_response');
    ...
    public function initiate($obj_id = null)
    {
        ...
        if (count((array) $dbRow)) {
            ...
            if (!empty($this->session->userdata('role'))) {
                ...
                $this->load->view('kaac/first_group/kiosk_payment', $data);
        ...
        if (count((array) $dbRow)) {
            ...
            if ($this->slug === "user") {
                $data['department_data'] = array(
                    "DEPT_CODE" => "ITD", //$dbRowGras->DEPT_CODE,
                    "OFFICE_CODE" => "ITD009", //$dbRowGras->OFFICE_CODE,
                    ...
                    "SUB_SYSTEM" => "ARTPS-SP|" . base_url('spservices/kaac/Payment_response/response'),
