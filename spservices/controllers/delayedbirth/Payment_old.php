...
class Payment extends Rtps
{
  ...
  private function my_transactions()
  {
    ...
    if (isset($user['role']) && !empty($user['role'])) { //PFCs and CSCs
      redirect(base_url('iservices/admin/my-transactions'));
    } else { //Citizens has no role!
  ...
  public function initiate($obj_id = null)
  {
    ...
    if ($obj_id) {
      ...
      //POST to the Backend Edistrict API if pfc_payment_status already set as "Y"
      if (property_exists($dbRow, 'pfc_payment_status') && $dbRow->form_data->pfc_payment_status == 'Y') {
        $this->application_submission($obj_id, "PDBR"); //POST to the Backend Edistrict API
        $this->session->set_flashdata('message', 'Payment already and hence the status has updated');
        $this->my_transactions();
      }

      //GET GRN call, if payment already initiated
      if (!empty($dbRow->form_data->payment_params) && !empty($dbRow->form_data->department_id)) {
        ...
        if ($res) {
          $this->application_submission($obj_id, "PDBR");
        }
      }

      //New Payment
      $filter = array("_id" => new ObjectId($obj_id));
      $this->delayed_birth_model->update_where($filter, array('service_data.appl_status' => 'payment_initiated'));
      // pre($filter);
      // die;
  ...
  private function pfcpayment()
  {
    ...
    if ($this->input->post('objid')) {
      $obj_id = $this->input->post('objid');
      $filter = array("_id" => new ObjectId($obj_id));
      $transaction_data = $this->delayed_birth_model->get_row($filter);
  ...
  public function update_query_payment_amount($data)
  {
    ...
    $data_to_update = array(
      'query_department_id' => $payment_params['DEPARTMENT_ID'],
