...
class Actions extends Upms
{
    ...
    public function app_pre_admin($objId = null)
    {
        ...
        $checkSubCodeFieldsAvailability = false;
        ...
        $this->load->view('upms/includes/header');
        $this->load->view('duplicatecertificateahsec/edit_dc_app_admin_pre', array('dbrow' => $dbRow, 'reg_data' => $reg_data, 'marksheet_data' => $ahsecmarksheet_data, 'commencing_dates' => $commencing_dates, 'subjects' => $subjects, 'marksheet_data_cnt' => $marksheet_data_cnt, 'reg_data_cnt' => $reg_data_cnt, 'sessions' => $sessions, 'checkSubCodeFieldsAvailability' => $checkSubCodeFieldsAvailability));
        $this->load->view('upms/includes/footer');
    }
    ...
    public function preview($objId = null)
    {
        ...
        if (count((array) $dbRow)) {
            ...
            if (($dbRow->service_data->service_id != "AHSECDRC") && ($dbRow->service_data->service_id != "AHSECDMRK")) {
                ...
                $master_data1 = $this->ahsecmarksheet_model->get_row($filter);
            }
