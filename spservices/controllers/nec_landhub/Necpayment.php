...
class Necpayment extends Rtps {
    ...
    private function check_pfc_payment_status($DEPARTMENT_ID = null) {
        if ($DEPARTMENT_ID) {
            ...
            $ref = modules::load('spservices/nec_landhub/payment_response');
    ...
    public function application_submission($obj_id) {
        ...
        $dbrow = $this->necertificates_model->get_row(['_id' => new ObjectId($obj_id)]);
        $ref = modules::load('spservices/nec_landhub/necresponse');
    ...
    public function initiate($obj_id = null) {
        ...
        if($dbRow) {
            ...
            $this->necertificates_model->update_where(['_id' => new ObjectId($obj_id)], $payData); //Update status            
            ...
            if (!empty($this->session->userdata('role'))) {
                ...
                $this->load->view('nec_landhub/necpayment_view', $data);
    ...
    private function payment_make($obj_id = null) {
        $dbRow = $this->necertificates_model->get_by_doc_id($obj_id); //pre($dbRow);
                ...
                if (strlen($treasuryCode) && strlen($officeCode)) {
                    ...
                    if ($serviceRow) {
                        ...
                        */
                        ...
                        //New Fees
                        $yearFee = ($yearDiff > 0) ? ($yearDiff*20) : 0;
                        $chargeFee = 20+$yearFee;//2020 to 2021 == 2 years fee not 1 years;
                        //Fee calculation end
                        ...
                        $data['department_data'] = array(
                            ...
                            "TAX_ID" => "",
                            "PAN_NO" => "",
                            "PARTY_NAME" => isset($dbRow->applicant_name) ? $this->clean($dbRow->applicant_name) : "RTPS TEAM", //"TEAM IGR", //applicant name , address of client
                            ...
                            "SUB_SYSTEM" => "ARTPS-SP|" . base_url('spservices/nec_landhub/necresponse/paymentmade'),
                        );
                    ...
                    $this->load->view('includes/frontend/header');
                    $this->load->view('nec_landhub/custominfo_view', $data);
                    $this->load->view('includes/frontend/footer');
                }//End of if else
            } else {
                $data = array(
