...
class Necresponse extends frontend {
    ...
    public function paymentmade() {
        ...
        if ($_POST['STATUS'] === 'Y') {
            ...
            if (true) { //Only for testing pupose
    ...
    public function post_data($department_id = null) {
        ...
        if ($dbrow) {
            ...
            $data["Ref"] = array(
                ...
                "service_mode" => ($dbrow->delivery_mode === 'delivery_urgent')?'U':'G',
                "fee_paid" => $total,
                "circle" => $dbrow->circle,
            ...
            //$url = $this->config->item('url'); //Fetch from spconfig
            //$url = 'https://ngdrs.assam.gov.in/NGDRS_ASM_DEMO/'; //For NGDRS server
            $url = 'https://landhub.assam.gov.in/webapi_test/webapi_land/'; //For landhub
