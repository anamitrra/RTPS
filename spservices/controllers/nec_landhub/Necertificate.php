...
class Necertificate extends Rtps {
    ...
    public function submit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (strlen($objId)) {
                ...
                redirect('spservices/nec_landhub/necertificate/fileuploads/' . $objId);
            } else {
                $insert = $this->necertificates_model->insert($inputs);
                if ($insert) {
                    $objectId = $insert['_id']->{'$id'};
                    $this->session->set_flashdata('success', 'Your application has been successfully submitted');
                    redirect('spservices/nec_landhub/necertificate/fileuploads/' . $objectId);
                    exit();
                } else {
    ...
    public function fileuploads($objId = null) {
        if ($this->checkObjectId($objId)) {            
            ...
            if ($dbRow) {
                ...
                $this->load->view('includes/frontend/header');
                $this->load->view('nec_landhub/necertificateuploads_view', $data);
                $this->load->view('includes/frontend/footer');
            } else {
    ...
    public function submitfiles() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->session->set_flashdata('success', 'Your application has been successfully submitted');
            redirect('spservices/nec_landhub/necertificate/preview/' . $objId);
        } //End of if else
    }//End of submitfiles()
    ...
    public function preview($objId = null) {
        ...
        if (count((array) $dbRow)) {
            ...
            $this->load->view('includes/frontend/header');
            $this->load->view('nec_landhub/necertificatepreview_view', $data);
            $this->load->view('includes/frontend/footer');
        } else {
            $this->session->set_flashdata('error', 'No records found against object id : ' . $objId);
            redirect('spservices/nec_landhub/necertificate/');
    ...
    public function query($objId = null) {
        if ($this->checkObjectId($objId)) {            
            ...
            if ($dbRow) {
                ...
                $data["sro_dist_list"] = $this->sros_model->sro_dist_list();
                $this->load->view('includes/frontend/header');
                $this->load->view('nec_landhub/necertificatequery_view', $data);
                $this->load->view('includes/frontend/footer');
            } else {
    ...
    public function track($objId = null) {
        ...
        if (count((array) $dbRow)) {
            ...
            $this->load->view('includes/frontend/header');
            $this->load->view('nec_landhub/necertificatetrack_view', $data);
            $this->load->view('includes/frontend/footer');
        } else {
            $this->session->set_flashdata('error', 'No records found against object id : ' . $objId);
            redirect('spservices/nec_landhub/necertificate/');
