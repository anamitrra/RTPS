    ...
    public function submit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (strlen($objId)) { 
                ...
                redirect('spservices/appointmentbooking/registration/preview/' . $objId);
            } else {
                $service_data = array(
                ...
                if ($insert) {
                    ...
                    if ($submitMode === 'SAVE_NEXT') {
                        ...
                        redirect('spservices/appointmentbooking/registration/preview/' . $objectId);
                    } else {
    ...
    public function preview($objId = null) {
        if ($this->checkObjectId($objId)) {
            ...
            if (count((array) $dbRow)) {
                ...
                $this->load->view('includes/frontend/header');
                $this->load->view('appointmentbooking/preview_view', $data);
                $this->load->view('includes/frontend/footer');
            } else {
    ...
    public function acknowledgement($objId = null) {
        if ($this->checkObjectId($objId)) {
            ...
            if ($dbRow) {
                ...
                $this->load->view('includes/frontend/header');
                $this->load->view('appointmentbooking/acknowledgement_view', $data);
                $this->load->view('includes/frontend/footer');
            } else {
    ...
    public function track($objId = null) {        
        if ($this->checkObjectId($objId)) {
            ...
            if(count((array)$dbRow)) {
                ...
                $this->load->view('includes/frontend/header');
                $this->load->view('appointmentbooking/track_view', $data);
                $this->load->view('includes/frontend/footer');
            } else {
                $this->session->set_flashdata('error','No records found against object id : '.$objId);
                redirect('spservices/appointmentbooking');
    ...
    public function delivered($objId = null) {        
        if ($this->checkObjectId($objId)) {
            ...
            if(count((array)$dbRow)) {
                ...
                $this->load->view('includes/frontend/header');
                $this->load->view('appointmentbooking/delivered_view', $data);
                $this->load->view('includes/frontend/footer');
            } else {
                $this->session->set_flashdata('error','No records found against object id : '.$objId);
