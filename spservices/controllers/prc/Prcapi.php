...
class Prcapi extends frontend {
    ...
    public function update_data() { 
        ...
        $dbrow = $this->applications_model->get_row(array('service_data.appl_id'=>(int)$applId));
        ...
        if(count((array)$dbrow)) {
            ...
            if($status === 'QS') {
                ...
                $this->applications_model->update_where(['_id' => new ObjectId($dbrow->_id->{'$id'})], $data);

                //Sending Query SMS
                $sms=array(
                    "mobile" => (int)$dbrow->form_data->mobile_number,
                    "applicant_name" => $dbrow->form_data->applicant_name,
                    "service_name" => 'Application for Permanent Residence Certificate for Higher Education',
                ...
                if (!is_dir($dirPath)) {
                    ...
                    file_put_contents($dirPath . "index.html", '<html><head></head><body>This directory contains files for Application for Permanent Residence Certificate only</body></html>');
                }
                ...
                $sms=array(
                    "mobile" => (int)$dbrow->form_data->mobile_number,
                    "applicant_name" => $dbrow->form_data->applicant_name,
                    "service_name" => 'Application for Permanent Residence Certificate',
                    "submission_date" => date("d-m-Y h:i a", strtotime($this->mongo_db->getDateTime($dbrow->service_data->submission_date))),
                    "app_ref_no" => $dbrow->service_data->appl_ref_no,
                );
                sms_provider("delivery",$sms);
                // pre($this->digilocker_api->pushUriDigilocker($dbrow->form_data->mobile_number, $dbrow->form_data->rtps_trans_id));
                // pre($this->digilocker_api->pushUriDigilocker('9101379463', $dbrow->service_data->appl_ref_no));
                $resPost = array(
                    'encryptKey' => $this->input->post("encryptKey"),
                    'status' => "true"
                );
