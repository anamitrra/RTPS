...
class Payment_query_response extends Frontend
{
    ...
    public function response()
    {
        ...
        $response = $_POST;
        $this->tradelicence_model->update_row(array('form_data.query_department_id' => $DEPARTMENT_ID), array("form_data.query_payment_status" => $_POST['STATUS'], "form_data.query_payment_response" => $response));
        if ($_POST['STATUS'] === 'Y') {
            //check the grn for valid transactions
            if ($this->checkgrn($DEPARTMENT_ID, true) === $_POST['GRN']) {
                $transaction_data = $this->tradelicence_model->get_row(array('form_data.query_department_id' => $DEPARTMENT_ID));
                echo ($transaction_data->service_data->service_id);
                if($transaction_data->service_data->service_id === $this->serviceId){
                    $ref=modules::load('spservices/trade_licence/application');
                    $ref->querypaymentsubmit($transaction_data->_id->{'$id'},true);
                ...
                $this->tradelicence_model->update_row(array('form_data.query_department_id' => $DEPARTMENT_ID), array("form_data.query_payment_status" => "P", "form_data.query_payment_response.STATUS" => 'P'));
                $this->show_error();
            }
