...
class PaymentResponse extends Frontend
{
    ...
    public function response()
    {
        ...
        $response = $_POST;
        $this->tradelicence_model->update_where(['form_data.department_id' => $DEPARTMENT_ID], array("form_data.payment_status" => $_POST['STATUS'], "form_data.pfc_payment_response" => $response));
        if ($_POST['STATUS'] === 'Y') {
            if ($this->checkgrn($DEPARTMENT_ID, true) === $_POST['GRN']) {
                ...
                // );
                // $this->ncl_model->update_where(['department_id' => $DEPARTMENT_ID], $data);
                ...
                $transaction_data = $this->tradelicence_model->get_row(array('form_data.department_id' => $DEPARTMENT_ID));
                if ($transaction_data->service_data->service_id === "NC-TL") {
                    //pre("OK");
                    $ref = modules::load('spservices/trade_licence/Application');
                    $ref->finalsubmition($transaction_data->_id->{'$id'}, true);
                    //redirect(base_url('spservices/trade_permit/Acknowledgement/acknowledgement/') . $transaction_data->_id->{'$id'});
                }
                ...
                $this->tradelicence_model->update_where(['department_id' => $DEPARTMENT_ID], ["status" => "PAYMENT_COMPLETED", "payment_status" => "PAYMENT_FAILED"]);
                $this->show_error();
            } //End of if else
        } else {
            $this->show_error();
    ...
    { // TODO: need to check which are params to update
        ...
        if ($DEPARTMENT_ID) {
            ...
            if ($res) {
                ...
                //if($STATUS === "Y"){
                $this->tradelicence_model->update_row(
    ...
    public function cin_response()
    {
        if (!empty($_POST)) {
            if (!empty($_POST['DEPARTMENT_ID'])) {
                ...
                $this->tradelicence_model->update_row(array('department_id' => $DEPARTMENT_ID), array(
                ...
                if ($STATUS === 'Y') {
                    $transaction_data = $this->tradelicence_model->get_row(array('department_id' => $DEPARTMENT_ID));
                    $ref = modules::load('spservices/trade_licence');
    ...
    public function acknowledgement($objId = null)
    {
        ...
        if (count((array)$dbRow)) {
            ...
            $this->load->view('includes/frontend/header');
            $this->load->view('trade_licence/paymentacknowledgement_view', $data);
            $this->load->view('includes/frontend/footer');
        } else {
