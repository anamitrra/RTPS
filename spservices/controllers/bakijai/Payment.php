...
class Payment extends Rtps
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('bakijai/gras_config_model');
    ...
    public function application_submission($obj_id, $service_id)
    {
        if ($service_id === $this->serviceId) {
            $ref = modules::load('spservices/bakijai/bakcl');
    ...
    private function check_pfc_payment_status($DEPARTMENT_ID = null)
    {
        if ($DEPARTMENT_ID) {
            ...
            $ref = modules::load('spservices/bakijai/Payment_response');
    ...
    {
        $dbRow = $this->bakijai_model->get_by_doc_id($obj_id);
        $dbRowGras = $this->gras_config_model->get_row(array("eDISTRICT_DISTRICT" => $dbRow->form_data->district));
        //pre($dbRowGras);
        if (empty($dbRowGras)) {

            $this->session->set_flashdata('error', 'District (' . $dbRow->form_data->district . ') is not activated for payment!');
            redirect('spservices/bakijai/bakcl/preview/' . $obj_id);
        } else if (empty($dbRowGras->OFFICE_CODE)) {

            $this->session->set_flashdata('error', 'District (' . $dbRowGras->eDISTRICT_DISTRICT . ') is not activated for payment!');
            redirect('spservices/bakijai/bakcl/preview/' . $obj_id);
        }
        ...
        if (count((array) $dbRow)) {
            ...
            $this->bakijai_model->update_where(['_id' => new ObjectId($obj_id)], array('service_data.appl_status' => 'payment_initiated')); //Update status            
            ...
            if (!empty($this->session->userdata('role'))) {
                ...
                if ($this->slug === "CSC") {
                    $data['bakijai_certificate_charge'] = $this->config->item('bakijai_certificate_charge');
                } else {
                    $data['bakijai_certificate_charge'] = $this->config->item('bakijai_certificate_charge');
                }
                ...
                $this->load->view('bakijai/kiosk_payment', $data);
    ...
    private function payment_make($obj_id = null)
    {
        ...
        if (count((array) $dbRow)) {
            ...
            if ($this->slug === "user") {
                $application_charge = $this->config->item('bakijai_certificate_charge');
                $data['department_data'] = array(
                    "DEPT_CODE" => $dbRowGras->DEPT_CODE,
                    "PAYMENT_TYPE" => $dbRowGras->PAYMENT_TYPE,
                    "TREASURY_CODE" => $dbRowGras->TREASURY_CODE,
                    ...
                    "MAJOR_HEAD" => $dbRowGras->MAJOR_HEAD,
                    "AMOUNT1" => $application_charge,
                    "HOA1" => $dbRowGras->HOA1,
                    "CHALLAN_AMOUNT" => $application_charge,
                    "TAX_ID" => "",
                    "PAN_NO" => "",
                    "DEPARTMENT_ID" => $DEPARTMENT_ID,
                    "MOBILE_NO" => $dbRow->form_data->mobile,
                    "SUB_SYSTEM" => "ARTPS-SP|" . base_url('spservices/bakijai/Payment_response/response'),
    ...
    public function update_pfc_payment_amount($data, $application_charge)
    {
        ...
        $result = $this->bakijai_model->update_payment_status($appl_ref_no, $data_to_update);
    ...
    public function verify($obj_id = null)
    {
        ...
        if ($obj_id) {
            ...
            $application = $this->bakijai_model->get_row($filter);
