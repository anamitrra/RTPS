...
class Paymentresponse extends Frontend
{
    ...
    {   
       ...
       // pre($DEPARTMENT_ID);
        $response = $_POST;
        //echo($response);
        $this->licence_model->update_row(array('form_data.department_id' => $DEPARTMENT_ID), array("form_data.pfc_payment_status" => $_POST['STATUS'], "form_data.pfc_payment_response" => $response));
        if ($_POST['STATUS'] === 'Y') {
            //check the grn for valid transactions
            if ($this->checkgrn($DEPARTMENT_ID, true) === $_POST['GRN']) {
                //pre($DEPARTMENT_ID);
                $transaction_data = $this->licence_model->get_row(array('form_data.department_id' => $DEPARTMENT_ID));
                if ($transaction_data->service_data->service_id === "TRADE") {
                    //pre("OK");
                    $ref = modules::load('spservices/tradelicence/application');
                $ref->finalsubmition($transaction_data->_id->{'$id'}, true);
                redirect(base_url('spservices/tradelicence/acknowledgement/index/') . $transaction_data->_id->{'$id'});
                }
                // if ($transaction_data->service_data->service_id === "TRADE") {
                //     $ref = modules::load('spservices/tradelicence/application');
                ...
                $this->licence_model->update_row(array('form_data.department_id' => $DEPARTMENT_ID), array("form_data.pfc_payment_status" => "P", "form_data.pfc_payment_response.STATUS" => 'P'));
                $this->show_error();
            }
    ...
    public function cin_response()
    {
        ...
        if (!empty($_POST)) {
            if (!empty($_POST['DEPARTMENT_ID'])) {
                ...
                $this->licence_model->update_row(array('form_data.department_id' => $DEPARTMENT_ID), array(
