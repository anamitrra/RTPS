...
class Payment extends Rtps
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('castecertificate/gras_config_model');
    ...
    public function application_submission($obj_id,$service_id){
        if($service_id === $serviceId){
            $ref=modules::load('spservices/castecertificate/registration');
    ...
    private function check_pfc_payment_status($DEPARTMENT_ID = null) {
        if ($DEPARTMENT_ID) {
            ...
            $ref = modules::load('spservices/castecertificate/Payment_response');
    ...
    public function initiate($obj_id = null) {
        ...
        $dbRowGras = $this->gras_config_model->get_row(array("district" => $dbRow->form_data->district));
        //pre($dbRowGras);
        if (empty($dbRowGras)) {

            $this->session->set_flashdata('error', 'District (' . $dbRow->form_data->district . ') is not activated for payment!');
            redirect('spservices/castecertificate/registration/preview/' . $obj_id);
        }
        ...
        if (count((array) $dbRow)) {
            ...
            if (!empty($this->session->userdata('role'))) {
                ...
                $this->load->view('castecertificate/kiosk_payment', $data);
        ...
        if (count((array) $dbRow)) {
            ...
            if ($this->slug === "user") {
                ...
                $data['department_data'] = array(
                    ...
                    "SUB_SYSTEM" => "ARTPS-SP|" . base_url('spservices/castecertificate/Payment_response/response'),
                    ...
                    "ACCOUNT1" => "PFC23362", //$user->account1,$account = "PFC23362";
                ...
                $data['department_data'] = array(
                    ...
                    "MULTITRANSFER" => "Y",
                    "NON_TREASURY_PAYMENT_TYPE" => $dbRowGras->payment_type,
                    //"TOTAL_NON_TREASURY_AMOUNT" => $total_amount + $application_charge + 10,
                    //"AC1_AMOUNT" => $application_charge,
