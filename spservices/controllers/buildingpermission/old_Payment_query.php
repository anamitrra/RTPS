...
class Payment_query extends Rtps
{
    ...
    public function initiate($obj_id = null) {
        ...
        if ($dbRow->form_data->frs_request->bpPaymentStatus == "UNPAID"){
            $dbRowBpGras = $this->gras_config_model->get_row(array("ulb_panchayat_id" => (string)$dbRow->form_data->frs_request->ulbPanchayatIdForBp));
        ...
        if ($dbRow->form_data->frs_request->ppPaymentStatus == "UNPAID"){
            $dbRowPpGras = $this->gras_dev_auth_config_model->get_row(array("development_authority_id" => (string)$dbRow->form_data->frs_request->developmentAuthorityIdForPP));
