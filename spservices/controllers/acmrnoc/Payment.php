...
{
    private $serviceId = "ACMRNOC";
    private $convenience_fee;
    private $acmr_noc_application_fee;
    ...
    public function __construct()
    {
        ...
        $this->acmr_noc_application_fee = $this->config->item('acmr_noc_application_fee');
    } //End of __construct()

    public function application_submission($obj_id, $service_id)
    {
        if ($service_id === $this->serviceId) {
            $ref = modules::load('spservices/acmrnoc/noc');
    ...
    private function check_pfc_payment_status($DEPARTMENT_ID = null)
    {
        if ($DEPARTMENT_ID) {
            ...
            $ref = modules::load('spservices/acmrnoc/Payment_response');
    ...
    public function initiate($obj_id = null)
    {
        ...
        $dbRow = $this->acmrnoc_model->get_by_doc_id($obj_id);
        if (count((array) $dbRow)) {
            ...
            $this->acmrnoc_model->update_where(['_id' => new ObjectId($obj_id)], array('service_data.appl_status' => 'payment_initiated')); //Update status            
            ...
            if (!empty($this->session->userdata('role'))) {
                ...
                $data['acmr_noc_application_fee'] = $this->acmr_noc_application_fee;
                $data['objid'] = $obj_id;
                ...
                $this->load->view('acmrnoc/kiosk_payment', $data);
    ...
    private function payment_make($obj_id = null)
    {
        ...
        if (count((array) $dbRow)) {
            ...
            $dept_code = 'DME';
            $office_code = "DME000";
            $acmr_noc_account_code = "DME91079";
            ...
            if ($this->slug === "user") {
                $data['department_data'] = array(
                    ...
                    "SUB_SYSTEM" => "ARTPS-SP|" . base_url('spservices/acmrnoc/Payment_response/response'),
                    ...
                    "MULTITRANSFER" => "Y",
                    "NON_TREASURY_PAYMENT_TYPE" => "04",
                    "TOTAL_NON_TREASURY_AMOUNT" => $this->convenience_fee + $this->acmr_noc_application_fee,
                    "AC1_AMOUNT" => $this->acmr_noc_application_fee,
                    "ACCOUNT1" => $acmr_noc_account_code,
                    "AC2_AMOUNT" => $this->convenience_fee,
                    "ACCOUNT2" => $this->convenience_fee_account, //this is convenient fee Account code from spconfig file
                );
                // pre($data);
            } else {
                if ($this->slug === "CSC") {
                ...
                if ($this->slug === "CSC") {
                    $data['department_data']['TOTAL_NON_TREASURY_AMOUNT'] = $total_amount + $this->acmr_noc_application_fee + $this->convenience_fee;
                    $data['department_data']['AC1_AMOUNT'] = $this->acmr_noc_application_fee;
                    $data['department_data']['ACCOUNT1'] = $acmr_noc_account_code;
    ...
    public function update_pfc_payment_amount($data)
    {
        ...
        $data_to_update['form_data.application_charge'] = $this->acmr_noc_application_fee;

        $result = $this->acmrnoc_model->update_payment_status($appl_ref_no, $data_to_update);
    ...
    public function verify($obj_id = null)
    {
        ...
        if ($obj_id) {
            ...
            $application = $this->acmrnoc_model->get_row($filter);
