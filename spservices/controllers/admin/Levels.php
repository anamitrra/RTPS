...
class Levels extends admin {
    ...
    public function submit(){
        ...
        $this->form_validation->set_rules('level_no', 'Level no.', 'required|max_length[255]');
        $this->form_validation->set_rules('level_roles', 'Role', 'required|max_length[255]');
        $this->form_validation->set_rules('level_name', 'Level name', 'required|max_length[255]');
    ...
    public function get_role_list(){
        $service_code = $this->input->post('service_code');
        $res = $this->levels_model->get_role_from_level($service_code);
        $data = array();
        foreach ($res as $row) {
            $data[] = array("role_code" => $row->role_code, "role_name" => $row->role_name);
        } //End of foreach()
