...
{   
    ...
    public function application_rejected(){
       ...
       if( $res->getMatchedCount()){
        ...
        ->set_content_type('application/json')
        ->set_status_header(200)
        ->set_output(json_encode(array("status"=>true,"message"=>"Status updated successfully")));

       }else{
        return $this->output
        ->set_content_type('application/json')
        ->set_status_header(200)
        ->set_output(json_encode(array("status"=>false,"message"=>"Unable to update status. Please try again.")));
    ...
    public function push_query_payment_status(){
        ...
        if($rtps_trans_id){
            ...
            if(property_exists($transaction_data,'query') && property_exists($transaction_data,'query_payment_response')  && $transaction_data->query_payment_status === 'Y' ){
                ...
                $url="https://ngdrs.assam.gov.in/NGDRS_ASM_DEMO/CertifiedCopy/getPaymentResponse";//$this->config->item('url');
                $curl = curl_init($url);
