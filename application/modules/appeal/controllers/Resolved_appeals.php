...
class Resolved_appeals extends Rtps
{
    ...
    public function index()
    {
        ...
        $this->load->view('resolved_appeals/index');
...
//
     ...
     * @return void
     */
    public function get_resolved_appeals($appealType = 'first')
    {
        $this->load->model("resolved_appeals_model");
        $columns = array(
            0 => "initiated_data.appl_ref_no",
            1 => "initiated_data.applied_by",
            2 => "initiated_data.submission_date",
        ...
        $order = $columns[$this->input->post("order")[0]["column"]];
        $dir = $this->input->post("order")[0]["dir"];
        $totalData = $this->resolved_appeals_model->total_rows();
        $totalFiltered = $totalData;
        $filter = array();
        if (empty($this->input->post("search")["value"])) {
            $this->session->set_userdata($filter);
            $records = $this->resolved_appeals_model->appeals_filter($limit, $start, $filter, $order, $dir);
            $totalFiltered = $this->resolved_appeals_model->appeals_filter_count();
        } else {
            $search = trim($this->input->post("search")["value"]);
            $records = $this->resolved_appeals_model->appeals_search_rows($limit, $start, $search, $order, $dir);
            $totalFiltered = $this->resolved_appeals_model->appeals_tot_search_rows($search);
        }
        $data = array();
        //pre($records);
        if (!empty($records)) {
