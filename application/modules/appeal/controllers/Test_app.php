...
class Test_app extends Rtps
{
    ...
    public function sign_doc($type,$encoded_filepath = null)

    {
        ...
        // $this->load->view('includes/frontend/header');
        $this->load->view('includes/header', array("pageTitle" => "Dashboard | Sign Order"));
        $this->load->view('test/digital_sign1',array('type'=>$type,'pdf_path'=>$pdf_path,'pdf_data'=>$b64_pdf));
        $this->load->view('includes/footer');

    }

    public function sign_doc2($encoded_filepath = null)
    {
        if($this->input->method() == 'post'){
            // $pdf_path = 'data:application/pdf;base64,'.$this->input->post('base_64_pdf');
            $b64_pdf = $this->input->post('base_64_pdf');
...
if (strpos($b64_pdf, 'data:application/pdf;base64,') !== false) {
    $b64_pdf = str_replace('data:application/pdf;base64,', '', $b64_pdf);
}

// Decode base64 string
$pdf_data = base64_decode($b64_pdf);

// Specify the output file path
// $output_path = FCPATH.'storage/DOCUMENTS/secondappeal/'uniqid()'.pdf';
...
// readfile($output_path);
        ...
        // $this->load->view('includes/frontend/header');
        $this->load->view('includes/header', array("pageTitle" => "Dashboard | Sign Order"));
        $this->load->view('test/digital_sign2',array('pdf_path'=>base_url ($dir.$output_filename),'pdf_data'=>$this->input->post('base_64_pdf')));
        $this->load->view('includes/footer');
...
public function ds_ho_app_2($encoded_filepath = null)
    {
        if($this->input->method() == 'post'){
            $b64_pdf = $this->input->post('base_64_pdf');
...
file_put_contents($output_path, $pdf_data);
        ...
        $this->load->view('includes/header', array("pageTitle" => "Dashboard | Sign Order"));
        $this->load->view('test/ds_ho_app_2',array('pdf_path'=>base_url ($dir.$output_filename),'pdf_data'=>$this->input->post('base_64_pdf')));
        $this->load->view('includes/footer');

    }

    public function ds_ho_dps_2($encoded_filepath = null)
    {
        if($this->input->method() == 'post'){
            $b64_pdf = $this->input->post('base_64_pdf');
...
file_put_contents($output_path, $pdf_data);
        ...
        $this->load->view('includes/header', array("pageTitle" => "Dashboard | Sign Order"));
        $this->load->view('test/ds_ho_dps_2',array('pdf_path'=>base_url ($dir.$output_filename),'pdf_data'=>$this->input->post('base_64_pdf')));
        $this->load->view('includes/footer');

    }


        public function ds_ro_2($encoded_filepath = null)
    {
        if($this->input->method() == 'post'){
            $b64_pdf = $this->input->post('base_64_pdf');
...
file_put_contents($output_path, $pdf_data);
        ...
        $this->load->view('includes/header', array("pageTitle" => "Dashboard | Sign Order"));
        $this->load->view('test/ds_ro_2',array('pdf_path'=>base_url ($dir.$output_filename),'pdf_data'=>$this->input->post('base_64_pdf')));
        $this->load->view('includes/footer');

    }


     public function ds_do_2($encoded_filepath = null)
    {
        if($this->input->method() == 'post'){
            $b64_pdf = $this->input->post('base_64_pdf');
...
file_put_contents($output_path, $pdf_data);
        ...
        $this->load->view('includes/header', array("pageTitle" => "Dashboard | Sign Order"));
        $this->load->view('test/ds_do_2',array('pdf_path'=>base_url ($dir.$output_filename),'pdf_data'=>$this->input->post('base_64_pdf')));
        $this->load->view('includes/footer');
...
//        $x = $this->pdf->base_64($latestHearingOrders->templateContent->appellant,'order');
    ...
    public function dsc_callback(){
        
            $actionTakenBy = $this->session->userdata('userId')->{'$id'};
            $type=$this->input->post('type');
            $appeal_id = $this->appeal_application_model->get_by_id($this->input->post('obj_id'));
            ...
            $inputs = [
                'appeal_id' => $appeal_id->appeal_id,
                'action' => $type.'-dsc-sign-generated',
                "message"=>"Signed Document for ".$type." successfully done",
                ...
                'dsc_file_'.$type=>$_POST['pdf_file'],
                'documents'=>$_POST['signed_pdf'],
                'created_at' => new UTCDateTime(strtotime(date("d-m-Y H:i:s")) * 1000),
            ];

           
            $this->load->model('appeal_process_model');
           $res = $this->appeal_process_model->insert($inputs);
           if($res){
            return $this->output
    ...
    public function dsc_callback_sec_app(){
        // pre($_POST);
        // return;
        $actionTakenBy = $this->session->userdata('userId')->{'$id'};
        $type=$this->input->post('type');
