...
class Department_wise_applications extends Rtps
{
    ...
    public function index(){
        ...
        $this->load->model('applications_model');
         $deliver = $this->applications_model->tot_search_rows(array(
            ...
            'Deliver',
            'initiated_data.department_id'=>'1234'
          ));
         $total = $this->applications_model->tot_search_rows(array(
            'initiated_data.department_id'=>'1234'
          ));
          ...
          $filter['department.department_id']='1234';
          $this->department_wise_application_model->update_where($filter,$data);
         
        $deptWiseApplications = $this->department_wise_application_model->get_all(array());
        $data = array(
            'deptWiseApplications' => $deptWiseApplications
        );
        $this->load->view('includes/header');
        $this->load->view('applications/department_wise',$data);
    ...
    public function show($deptId){
         ...
         $departmentInfo = $this->department_model->get_by_doc_id(new ObjectId($deptId));
    ...
    public function deppt()
    {
        ...
        //pre($data);
        $this->department_wise_application_model->insert($data);die;
        pre($data);
    }
}