...
class Locked_appeals extends Rtps
{
    ...
    public function index()
    {
        ...
        $this->load->view('locked_appeals/index');
...
//        if(count($appealApplication))
     ...
     * @return void
     */
    public function get_locked_appeals($appealType = 'first')
    {
        $this->load->model("locked_appeals_model");
        $columns = array(
        ...
        $totalData = $this->locked_appeals_model->total_rows();
        $totalFiltered = $totalData;
        $filter = array();
        if (empty($this->input->post("search")["value"])) {
            $this->session->set_userdata($filter);
            $records = $this->locked_appeals_model->appeals_filter($limit, $start, $filter, $order, $dir);
            $totalFiltered = $this->locked_appeals_model->appeals_filter_count();
        } else {
            $search = trim($this->input->post("search")["value"]);
            $records = $this->locked_appeals_model->appeals_search_rows($limit, $start, $search, $order, $dir);
            $totalFiltered = $this->locked_appeals_model->appeals_tot_search_rows($search);
        }
        $data = array();
        //pre($records);
        if (!empty($records)) {
            ...
            foreach ($records as $rows) {
                ...
                $btns = '<a href="#!" class="btn btn-sm btn-outline-primary" data-appl_ref_no="' . $rows->appeal_id . '" data-id="' . $rows->{'_id'}->{'$id'} . '" onclick="approveLockedApplications(this)">Approve</a>';
                $btns.='<a href="#!" class="btn btn-sm btn-outline-warning" data-appl_ref_no="' . $rows->appeal_id . '" data-id="' . $rows->{'_id'}->{'$id'} . '" onclick="rejectLockedApplications(this)">Reject</a>';
                $nestedData["sl_no"] = $sl;
    ...
    /**
     * approve_locked_application
     *
     * @return void
     */
    public function approve_locked_application(){
        if($this->input->is_ajax_request()){
            ...
            // change locked flag to false and set the reason
            ...
            $this->load->model("locked_appeals_model");
            $this->locked_appeals_model->update($appeal_id,$lockedAppealInputs);

            $appealApplication = $this->locked_appeals_model->get_by_doc_id($appeal_id);
            ...
            $msg = 'Locked appeal approved for processing. Appeal ID ' . $appeal_id . '.';
            $msg = urlencode($msg);
            foreach ($applicantNotifiable['mobile'] as $contactMobile) {
                $this->sms->send($contactMobile, $msg);
            }
            // send email
            $emailBody = '<p>' . 'Locked appeal approved for processing. Appeal ID ' . $appeal_id . '</p>';
            foreach ($applicantNotifiable['email'] as $contactEmail) {
                $this->remail->sendemail($contactEmail, "locked appeal approved", $emailBody);
            }
    ...
    /**
     * reject_locked_application
     *
     * @return void
     */
    public function reject_locked_application(){
        if($this->input->is_ajax_request()){
            $this->load->library('form_validation');
            ...
            // applicant notification
            $msg = 'Locked appeal Rejected for processing. Appeal ID ' . $appeal_id . '.';
            $msg = urlencode($msg);
            foreach ($applicantNotifiable['mobile'] as $contactMobile) {
                $this->sms->send($contactMobile, $msg);
            }
            // send email
            $emailBody = '<p>' . 'locked appeal Rejected for processing. Appeal ID ' . $appeal_id . '</p>';
            foreach ($applicantNotifiable['email'] as $contactEmail) {
                $this->remail->sendemail($contactEmail, "locked appeal approved", $emailBody);
            }
