...
<form id="resolveForm">
    ...
    <div class="card card-body mt-2 p-2 border border-warning shadow" style="flex: none" id="templateLinkBox">
        ...
        <div class="btn-group">
            <?=!$disposalOrder[0]?:'<a href="'.base_url($disposalOrder[0]).'" class="btn btn-outline-primary"  target="_blank">View Disposal Order</a>'?>
        </div>
    </div>
      ...
      <div class="btn-group mb-2">
            <?php
                if($disposalOrder){
                    ...
                    echo '<br/><a href="'.base_url('appeal/test_app/sign_doc/disposal-order/'. base64_encode($disposalOrder[0])).'" class="btn btn-outline-primary"  target="">DSC : Disposal Order</a>';
                }
            ?>
        </div>
    ...
    <div class="row mt-2">
        <div class="col-12">
            <button class="btn btn-success" id="processSubmit" type="button">
                ...
                <span id="submitBtnTxt"> <span class="fa fa-check"></span> Approve Final Verdict</span>
            </button>
        </div>
    </div>
...
<script>
    ...
    var FormProcessUrl = '<?= base_url('appeal/first/process/approve-disposal-order') ?>';
            ...
            Swal.fire({
                ...
                if (result.value) {
                    if (FormRef.parsley().validate()) {
                        ...
                        formData.push({name : 'disposalOrder', value : '<?=urlencode($disposalOrder[0])?>'});
                        $.ajax({
                            url: FormProcessUrl,
                            type: 'POST',
                            dataType: 'JSON',
