<form id="reassignForm">
    ...
    <div class="row  reassign-box mt-3">
        <div class="col-12">
            <label for="reassignTo">Reassign To (Another DPS)</label>
            <select class="select2" id="reassignTo" name="reassignTo" data-placeholder="Select an user" style="width: 100%;" data-parsley-errors-container="#reassignToErrorContainer">
                <?= $reAssignAbleUserOptionList ?? '<option value="">No user available</option>' ?>
            </select>
            <div id="reassignToErrorContainer"></div>
            <input type="hidden" name="reassignToRoleSlug" id="reassignToRoleSlug">
        </div>
    </div>
...
<script>
    ...
    var reassignFormRef = $('#reassignForm');
    var reassignFormProcessUrl = '<?= base_url('appeal/first/process/reassign') ?>';
    ...
    $(function() {
        ...
        processSubmitRef.click(function() {
            if (reassignFormRef.parsley().validate()) {
                var sweet_loader = '<div class="sweet_loader"><svg viewBox="0 0 140 140" width="140" height="140"><g class="outline"><path d="m 70 28 a 1 1 0 0 0 0 84 a 1 1 0 0 0 0 -84" stroke="rgba(0,0,0,0.1)" stroke-width="4" fill="none" stroke-linecap="round" stroke-linejoin="round"></path></g><g class="circle"><path d="m 70 28 a 1 1 0 0 0 0 84 a 1 1 0 0 0 0 -84" stroke="#71BBFF" stroke-width="4" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-dashoffset="200" stroke-dasharray="300"></path></g></svg></div>';
                $.ajax({
                    url: reassignFormProcessUrl,
                    type: 'POST',
                    dataType: 'JSON',
                    data: reassignFormRef.serialize(),
                    beforeSend: function(){
                        reassignFormRef.find(":input").prop("disabled", true);
                        swal.fire({
                    ...
                    reassignFormRef.find(":input").prop("disabled", false);
                    swal.close();
                });
            }
        });
