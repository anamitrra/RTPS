...
$encodedPdf = $pdf_data;//base64_encode(file_get_contents($pdfPath));
$pdf_path = $pdf_path;
// pre($pdf_path);
// return;

$obj_id=$this->session->userdata('doc_id');
            ...
            <div class="col-md-12">
                <div style="width: 100%;  border: 2px double #ccc; margin: 10px auto">
                    <div style=" background: #337ab7; margin: 5px; padding: 5px;">
                        ...
                        <a id="backtohome" href='<?= base_url('appeal/dashboard') ?>' type=""></a>
                        <a id="downloadDiv" href='#' type="application/pdf" download="SignedPdf.pdf"></a>

                    </div>                        
                    <embed src="<?=$pdf_path?>" type="application/pdf" frameBorder="0" scrolling="auto" height="500" width="100%"></embed>
                </div>
                <div class="well-sm">
                    <form id="pdfForm">
                        ...
                        <input type="hidden" id="stampingX" name="stampingX" value="150" />
                        <input type="hidden" id="stampingY" name="stampingY" value="530" />
                            ...
                            success: function(data) {
                                ...
                                if (data.status_cd == 1) {
                                    ...
                                    if (jsonData.status === "SUCCESS") {
                                        ...
                                        $.post("<?=base_url('appeal/test_app/dsc_callback_sec_app')?>", {
                                            "obj_id":"<?=$obj_id?>",
                                            "pdf_file":"<?=$pdf_path?>",
                                            "signed_pdf":pdfData,
                                            'type':"<?=$type?>"
