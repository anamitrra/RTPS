<?php
// $username = ""; // Initialize the variable (could be null or an empty string)
            ...
            <form action="<?php echo $action; ?>" method="post">
              <div class="row">
                ...
                <div class="form-group field-email col-md-6">
                  <label for="email" class="fb-text-label">Email <?php echo form_error('email') ?> </label>
                  <!-- <?php 
                        $disabled = $email? "disabled='disabled'" : "";
                  ?> -->
                  <!-- <input class="form-control" name="email" id="email"  value="<?= $email; ?>" required="required" aria-required="true" type="email" <?php echo $disabled; ?> > -->
                ...
                <div class="fb-text form-group col-md-6">
                  ...
                  <select class="form-control" id="roles" name="roleId" required>
                    <?php foreach ($roles as $role) { ?>
                      <option data-slug="<?=$role->slug?>" value="<?= $role->{'_id'}->{'$id'}; ?>" <?= $role->{'_id'}->{'$id'} == $roleId ? 'selected' : ''  ?>><?= $role->role_name; ?></option>
                    <?php } ?>
                  </select>
                </div>
                <input type="hidden" name="role_slug" id="role_slug"/>

                <div class="fb-text form-group col-md-6" id="datypeblock" style="display: none;">
                  <label for="roles" class="fb-text-label">DA Type <span class="fb-required text-danger">*</span></label>
                  <select class="form-control" id="da_type" name="da_type" >
                          <option value="">-Select-</option>
                          <option  <?=$da_type === "Appellate" ? 'selected':""?> value="Appellate">Appellate</option>
                          <option <?=$da_type === "Tribunal" ? 'selected':""?> value="Tribunal">Tribunal</option>
...
<!-- 
              <div class="row">
                <div class="fb-text form-group col-md-6">
                  <label for="username" class="fb-text-label">Username <?php echo form_error('username') ?> <span class="fb-required text-danger">*(Must be unique)</span></label>
                  <input class="form-control" name="username" id="username" value="<?= $username ?>" type="text" <?= ($username) ? 'readonly' : '' ?>>
                </div>
              </div> -->
              ...
              <div class="row">
                <div class="fb-text form-group col-md-6">
                  <label for="username" class="fb-text-label">Username <?php echo form_error('username') ?> <span class="fb-required text-danger">*(Must be unique)</span></label>
                  <input required class="form-control" name="username" id="username" value="<?= $username ?>" type="text" <?= ($username) ? 'readonly' : '' ?>>
                </div>
              </div>
