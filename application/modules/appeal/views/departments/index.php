                ...
                <form id="add_official_form" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="form-group">
                                <label for="department_id">LGD Department ID</label>
                                <input type="text" class="form-control" name="base_department_id" id="base_department_id" required>
                                <div class="text-danger error"></div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label for="department_name">Department Parent</label>
...
<script>
    $(document).ready(function(){
        ...
        var table = $('#departments').DataTable({
            ...
            "ajax": {
                url: "<?php echo site_url("appeal/departments/get_records") ?>",
                type: 'POST',
                beforeSend: function() {
        ...
        $(document).on('click', '#add_department_submit', function(){
            ...
            var base_department_id = modal.find('#base_department_id').val();
            var parent = modal.find('#parent').val();

            if(department_id == null || department_id == ''){
                modal.find('#department_id').addClass('is-invalid');
                modal.find('#department_id').next('.error').text('Plase insert department ID');
            }
            else if(base_department_id == null || base_department_id == ''){
                modal.find('#base_department_id').removeClass('is-invalid');
                modal.find('#base_department_id').next('.error').text('');
                modal.find('#base_department_id').addClass('is-invalid');
                modal.find('#base_department_id').next('.error').text('Plase insert LGD department id');
            }
                ...
                modal.find('#parent').removeClass('is-invalid');
                modal.find('#parent').next('.error').text('');
                modal.find('#parent').addClass('is-invalid');
                modal.find('#parent').next('.error').text('Plase select a parent');
            }else{
                modal.find('.is-invalid').removeClass('is-invalid');
                modal.find('.error').text('');
                $(this).text('Saving...');

                $.post('<?=base_url('appeal/department/add') ?>', {department_name:department_name,department_id:department_id,base_department_id:base_department_id,parent:parent}, function(data){
                    if(data.status == true){
                        table.draw();
                        modal.modal('hide');
                        Swal.fire({
        ...
        $(document).on('click', '.editDepartment', function() {
            ...
            swal.fire({
                ...
                onOpen: () => {
                    Swal.showLoading();
                    // var depertment_id = $(this).attr('data-id');
                    $.post('<?= base_url('appeal/departments/get_department_info') ?>', {
                        ...
                        if (jsn.status == true) {
                            ...
                            modal_box.find('#base_department_id').val(jsn.data.base_department_id);
                            modal_box.find('#department_name').val(jsn.data.department_name);
                            modal_box.find('#parent').val(jsn.data.parent);
                            ...
                            Swal.fire({
                                ...
                                title: 'Invalid Input',
                                text: 'Something went wrong! Please try again',
                                html:jsn.error_msg ?  jsn.error_msg : ''
                            })
                        }
                    })
                }
        ...
        $(document).on('click', '#update_department_submit', function(){
            ...
            var base_department_id =  modal_box.find('#base_department_id').val();
            var department_name =   modal_box.find('#department_name').val();
            var parent =   modal_box.find('#parent').val();
            ...
            if(department_name == null || department_name === ''){
                ...
                modal_box.find('#base_department_id').addClass('is-invalid');
                modal_box.find('#base_department_id').next('.error').text('Please insert base department id');
            }else if(parent == null || parent === ''){
                modal_box.find('#parent').addClass('is-invalid');
                modal_box.find('#parent').next('.error').text('Please select parent');
            }else{
                modal_box.find('.is-invalid').removeClass('is-invalid');
                modal_box.find('.error').text('');
                $(this).text('Updating..');

                $.post('<?=base_url('appeal/department/update') ?>', {dept_id:dept_id, department_id:department_id,base_department_id:base_department_id, department_name:department_name,parent:parent}, function(data){
                    if(data.status == true){
                        table.draw();
