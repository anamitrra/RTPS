...
class Transoprt_response extends Rtps
{
          ...
          //   $data['department_name']=$departmental_data->department_name;
            // $data['service_name']=$departmental_data->service_name;
             if(isset($response['ApplicationStatus']) && $response['ApplicationStatus'] === "S"){
               redirect(base_url('expr/admin/pfc-payment/').$rtps_trans_id);
             }else {
               $this->show_error();
             }
  ...
  public function payment_response(){
    ...
    if($_POST['STATUS'] === 'Y'){
      ...
      if($this->checkgrn($DEPARTMENT_ID,true) === $_POST['GRN']){
        redirect(base_url('expr/admin/get-acknowledgement?app_ref_no=').$DEPARTMENT_ID);
      }else {
      ...
      //  var_dump($STATUS);var_dump($GRN);die;
        //if($STATUS === "Y"){
          $this->intermediator_model->update_row(array('department_id'=>$DEPARTMENT_ID),array("pfc_payment_response.GRN"=>$GRN,
                                                                                  "pfc_payment_response.STATUS"=>$STATUS,
                                                                                  "pfc_payment_response.TAXID"=>$res[20],
                                                                                  //"pfc_payment_response.PRN"=>$res[12],
                                                                                  "pfc_payment_response.TRANSCOMPLETIONDATETIME"=>$res[14],
                                                                                  'pfc_payment_status'=>$STATUS));
          ...
          $result=$this->intermediator_model->add_param($response->rtps_trans_id,$data_to_update);
           if($result->getMatchedCount()){
             if(isset($response->status) && $response->status === "S"){
               redirect(base_url('expr/admin/pfc-payment/').$response->rtps_trans_id);
             }else {
               $this->show_error();
             }
           }else {
