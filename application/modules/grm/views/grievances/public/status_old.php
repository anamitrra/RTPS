        ...
        submitRef.click(function(){
            if(viewStatusFormRef.parsley().validate()){
                $.ajax({
                    ...
                    type: 'POST',
                    dataType: 'JSON',
                    data: viewStatusFormRef.serialize(),
                    beforeSend: function(){
                        swal.fire({
                    ...
                    success: function(response){
                        if(response.status){
                            ...
                            resDateOfReceiptRef.text(moment(response.data.DateOfReceipt,"DD-MM-YYYY") || 'NA');
                            ...
                            if(response.data.grievance_doc_name){
                                grievanceDocLink = '<a href="<?=base_url('storage/uploads/grm/attachments/')?>'+response.data.grievance_doc_name+'" class="btn btn-outline-warning" target="_blank">View</a>';
                            }
                            resGrievanceDocumentRef.html(grievanceDocLink || 'NA');
                            resCurrentStatusRef.text(response.data.CurrentStatus || 'NA');
                            resDateOfActionRef.text(moment(response.data.DateOfAction,"DD-MM-YYYY") || 'NA');
