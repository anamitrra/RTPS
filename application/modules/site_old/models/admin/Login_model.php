...
class Login_model extends Mongo_model
{
    ...
    {
        $select = array("_id", "name", "photo", "mobile", "email", "password", "roleId", "department", "designation");
    ...
    public function createPasswordUser($email, $password)
    {
        ...
        $this->mongo_db->where('email', trim($email));
        $this->mongo_db->update($this->table, $data);

        return [$password, getHashedPassword($password)];
    }
}
