...
class Epramaan extends External
{
    ...
    {
        if (isset($_GET["code"])) {
            if (!isset($_COOKIE["verifier_c"]) && (!isset($_COOKIE["nonce_c"]))) {
                header('Location: ' . base_url('iservices'));
            } else {
    ...
    public function verify_token($decryptedtoken)
    {
        ...
        if (!empty($this->session->userdata('applyBy'))) {
            ...
            } else if ($applyBy == 'citizen') {
                // echo 'citizen loginnnnnnn';
    ...
    public function pfc_login_process($data)
    {
        ...
        if (empty($user->email)) {
            $redirect_url = base_url('iservices/login/elogout');
