        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="javascript:void(0)" class="btn btn-primary btn-sm float-right" data-toggle="modal" data-target="#addRoleModal"><i class="fas fa-plus" aria-hidden="true"></i>&nbsp;
                        Add Role</a>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form id="add_role_form" method="POST">
                    ...
                    <div class="row">
                    <div class="col-md-12">
                    <label for="role_name">Role Access</label>
                    </div>
                    </div>
                    <div class="row">                        
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <div class="custom-control custom-checkbox">
                                        <input type="checkbox" id="customCheck<?= $key ?>" name="roles[]" class="custom-control-input" value="<?= $val->{'_id'}->{'$id'} ?>" >
                                        
                                        <label class="custom-control-label" for="customCheck<?= $key ?>"><?= $val->display_name ?></label>
                                    </div>
                                </div>
                            </div>
                        <?php endforeach; ?>
            ...
            <div class="modal-footer">
                ...
                <a href="javascript:void(0)" type="submit" id="add_role_submit" class="btn btn-primary float-right">Save</a>
            </div>
            </form>
        </div>
        <!-- /.modal-content -->
...
<script>
    $(document).ready(function() {
        ...
        var table = $('#departments').DataTable({
            ...
            "ajax": {
                url: "<?php echo base_url("mis/roles/get_records") ?>",
                type: 'POST',
                beforeSend: function() {
        ...
        $(document).on('click', '#add_role_submit', function() {
            ...
            if (role_name == null || role_name == '') {
                ...
                modal.find('#role_name').next('.error').text('Plase insert department ID');
            } else {
                modal.find('.is-invalid').removeClass('is-invalid');
                modal.find('.error').text('');
                $(this).text('Saving...');
                var data=$('#add_role_form').serializeArray();
                $.post('<?= base_url('mis/roles/add') ?>',data, function(data) {
                    if (data.status == true) {
                        table.draw();
                        modal.modal('hide');
                        Swal.fire({
        ...
        $(document).on('click', '.editRole', function() {
            ...
            swal.fire({
                ...
                onOpen: () => {
                    Swal.showLoading();
                    // var role_id = $(this).attr('data-id');
                    $.post('<?= base_url('mis/roles/get_role_info') ?>', {
                        'role_id': role_id,
                    }, function(jsn) {
                        if (jsn.status == true) {
                            ...
                            $(jsn.permissions).each(function(key,val){
                                ...
                                $('#permissions').append('<div class="col-md-4">\
                                <div class="form-group">\
                                    <div class="custom-control custom-checkbox">\
        ...
        $(document).on('click', '#update_role_submit', function() {
            ...
            if (role_name == null || role_name === '') {
                ...
                $.post('<?= base_url('roles/update') ?>', data, function(data) {
                    anc.text('Update');
                    if (data.status == true) {
                        table.draw();
