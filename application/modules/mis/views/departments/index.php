                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="form-group">
                                ...
                                <select type="text" class="form-control select2" name="department_user[]" id="department_user" multiple>
                                    <?php
                                        if(!empty($userList)){
                                            foreach ($userList as $user){
                                    ?>
                                    <option value="<?=$user->{'_id'}->{'$id'}?>"><?=$user->name?></option>
                                    <?php
                                            }
                                        }
                                    ?>
...
<script>
    $(document).ready(function(){
        ...
        var table = $('#departments').DataTable({
            ...
            "ajax": {
                url: "<?php echo base_url("mis/departments/get_records") ?>",
                type: 'POST',
                beforeSend: function() {
        ...
        $(document).on('click', '#add_department_submit', function(){
            ...
            var department_user = modal.find('#department_user').val();
            if(department_id == null || department_id == ''){
                ...
                modal.find('#department_name').next('.error').text('Please insert department name');
            }else{
                modal.find('.is-invalid').removeClass('is-invalid');
                modal.find('.error').text('');
                $(this).text('Saving...');

                $.post('<?=base_url('mis/department/add') ?>', {department_name:department_name,department_id:department_id, department_user: department_user}, function(data){
                    if(data.status == true){
                        table.draw();
                        modal.modal('hide');
                        Swal.fire({
        ...
        $(document).on('click', '.deleteDepartment', function(){
            ...
            $.post('<?=base_url('mis/department/delete') ?>', {'department_id' : department_id}, function(data){
                if(data.status == true){
                    table.draw();
                    Swal.fire({
                        position: 'top',
        ...
        $(document).on('click', '.editDepartment', function() {
            ...
            swal.fire({
                ...
                onOpen: () => {
                    Swal.showLoading();
                    // var depertment_id = $(this).attr('data-id');
                    $.post('<?= base_url('mis/departments/get_department_info') ?>', {
                        ...
                        if (jsn.status == true) {
                            ...
                            let department_users = [];
                            if(jsn.data.department_users !== undefined){
                                for(let i = 0; i < jsn.data.department_users.length; i++){
                                    department_users[i] = jsn.data.department_users[i].$oid;
                                }
                            }
                            modal_box.find('#department_user').val(department_users).trigger('change');
        ...
        $(document).on('click', '#update_department_submit', function(){
            ...
            var department_user =   modal_box.find('#department_user').val();
            ...
            if(department_name == null || department_name === ''){
                ...
                $.post('<?=base_url('mis/department/update') ?>', {dept_id:dept_id, department_id:department_id, department_name:department_name, department_user:department_user}, function(data){
                    if(data.status == true){
                        table.draw();
