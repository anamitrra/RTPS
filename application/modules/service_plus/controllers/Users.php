  ...
  */
  ...
  public function get_records() {
    ...
    if(!empty($designation)){
      $fillter=array('designation'=>$designation) ;
    }
    ...
    if (!empty($records)) {
      ...
      foreach ($records as $rows) {
        $objId = $rows->{"_id"}->{'$id'};
        $btns = '<a href="' . base_url("mis/users/read/$objId") . '" data-toggle="tooltip" data-placement="top" title="Edit" class="" ><span class="fa fa-eye" aria-hidden="true"></span></a> ';
        $btns .= '<a href="' . base_url("mis/users/update/$objId") . '" data-toggle="tooltip" data-placement="top" title="Edit" class=""><span class="fa fa-edit" aria-hidden="true"></span></a>';
        $btns .= '<a href="' . base_url("mis/users/delete/$objId") . '" data-toggle="tooltip" data-placement="top" title="Delete" class="" onclick="return confirm(\'Are You Sure you want to delete?\')"><span class="fas fa-trash" aria-hidden="true"></span></a>';
//        $dept_obj=$this->department_model->get_services($rows->department[0]);
        $dept_obj = 'NA';
        if(property_exists((object)$rows->department,'0'))
        $dept_obj = $this->department_model->first_where(['department_id' => $rows->department[0]]);
        $nestedData["userId"] = $sl;
        $nestedData["name"] = $rows->name;
        $nestedData["department"] = (property_exists($dept_obj,'department_name'))?$dept_obj->department_name:"NA";
  ...
  public function index() {
    $this->load->view('includes/header', array("pageTitle" => "Mis | Users"));
  ...
  public function create() {
    ...
    $data = array(
      'button' => 'Create',
      'action' => base_url('mis/users/create_action'),
    ...
    $this->load->view('includes/header', array('pageTitle' => 'Mis | Create User'));
  ...
  public function update($id) {
    ...
    if ($row) {
      $data = array(
        ...
        'button' => 'Update',
        'action' => base_url('mis/users/update_action'),
        'userId' => $row->{"_id"}->{'$id'},
        'email' => $row->email,
        'password' => '123456789',
        'name' => $row->name,
      ...
      //pre($data);
      $this->load->view('includes/header', array('pageTitle' => 'Mis | Update User'));
      $this->load->view('users/users_form', $data);
      $this->load->view("includes/footer");
    } else {
      $this->session->set_flashdata('message', 'Record Not Found');
...
//    }
  ...
  public function profile(){
      $this->load->view('includes/header', array('pageTitle' => 'Mis | Profile'));
