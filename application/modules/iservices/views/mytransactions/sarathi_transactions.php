                              ...
                              <td>
                                    <?php if(isset($value->applied_by)){ ?>
                                          <?php if ( isset($value->pfc_payment_status)  && $value->pfc_payment_status ==="Y"): ?>
                                            <a class="btn btn-primary btn-sm mbtn" href="<?=base_url()?>iservices/sarathi/check-status?rtps_trans_id=<?=$value->rtps_trans_id?>" >Track</a>
                                    ...
                                    <?php }else { ?>
                                            ...
                                            <?php if($value->status === "S"){?>
                                                             ...
                                                             <?php if (isset($value->convenience_fee_required) && $value->convenience_fee_required): ?>
                                                                      <?php if (isset($value->pfc_payment_status)  && $value->pfc_payment_status ==="Y"): ?>
                                                                        <a class="btn btn-success btn-sm mbtn" href="<?=base_url("iservices/sarathi-acknowledgement?app_ref_no=").$value->app_ref_no?>" >Acknowledgement</a>
                                                                          <?php if (!empty($value->app_ref_no) && $value->status !="F"): ?>
                                                                            <a class="btn btn-primary btn-sm mbtn" href="<?=base_url()?>iservices/sarathi/check-status?rtps_trans_id=<?=$value->rtps_trans_id?>" >Track</a>
                                                                          <?php endif; ?>
                                                                      <?php else: ?>
                                                                          <a class="btn btn-primary btn-sm mbtn" href="<?=base_url()?>iservices/convenience_fee/payment/<?=$value->rtps_trans_id?>" >Pay Or Verify convenience fee</a>
                                                                      <?php endif; ?>
                                                            <?php else: ?>
                                                                    <?php if (!empty($value->app_ref_no) && $value->status !="F"): ?>
                                                                      <a class="btn btn-primary btn-sm mbtn" href="<?=base_url()?>iservices/sarathi/check-status?rtps_trans_id=<?=$value->rtps_trans_id?>" >Track</a>
                                            ...
                                            <?php } else{ ?>
                                              ...
                                              <?php if (property_exists($value,"portal_payment_status") && $value->portal_payment_status === "Success" ): ?>
                                              <a class="btn btn-success btn-sm mbtn" href="<?=base_url("iservices/pay-acknowledgement?app_ref_no=").$value->app_ref_no?>" >Payment Acknowledgement</a>
                                             <?php endif; ?>

                                             <?php if ($value->status ==="F"): ?>
                                            <a class="btn btn-success btn-sm mbtn" href="<?=base_url("iservices/sarathi/guidelines/").$value->service_id."/".$value->portal_no?>" >Apply Again</a>
                                                <?php endif; ?>
                                                <?php if ($value->status ==="" || $value->status ==="P"): ?>
                                                  <a href="<?=base_url("iservices/sarathi/retry/").$value->rtps_trans_id."/".$value->service_id."/".$value->portal_no?>" class="btn btn-sm btn-primary mbtn blk">Complete Pending Application</a>
                                                <?php endif; ?>
