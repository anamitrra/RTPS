  ...
  <section class="track-data">
    ...
    <?php else: ?>
      ...
      <table class="table bordered">
        ...
        <?php
        if(!empty($result->task_details))
         foreach ($result->task_details as $key => $value): ?>
          <tr>
            <td><?=$key+1?></td>
            ...
            <td><?=$value->remark?></td>
            <td>
            <?=$value->action === "Complete" ? "Delivered" : $value->action ?>
              ...
              <?php if ($value->action === "Complete"): ?>
                <?php if(!empty($value->url)){ ?>
                   <a href="<?=$value->url?>?data=<?=$result->app_ref_no?>" class="btn btn-success btn-sm">Download</a>
                <?php }?>
