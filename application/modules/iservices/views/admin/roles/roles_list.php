...
<script>
    $(document).ready(function () {
        ...
        var table = $('#departments').DataTable({
            ...
            "ajax": {
                url: "<?php echo site_url("iservices/admin/roles/get_records") ?>",
                type: 'POST',
                beforeSend: function () {
        ...
        $(document).on('click', '#add_role_submit', function () {
            ...
            if (role_name == null || role_name === '') {
                ...
                $.post('<?= base_url('admin/roles/add') ?>', data, function (data) {
                    if (data.status == true) {
                        table.draw();
                        modal.modal('hide');
                        $('#add_role_form').reset();
        ...
        $(document).on('click', '.editRole', function () {
            ...
            swal.fire({
                ...
                onOpen: () => {
                    Swal.showLoading();
                    // var role_id = $(this).attr('data-id');
                    $.post('<?= base_url('admin/roles/get_role_info') ?>', {
                        'role_id': role_id,
                    }, function (jsn) {
                        if (jsn.status == true) {
        ...
        $(document).on('click', '#update_role_submit', function () {
            ...
            if (role_name == null || role_name === '') {
                ...
                $.post('<?= base_url('admin/roles/update') ?>', data, function (data) {
                    anc.text('Update');
                    if (data.status == true) {
                        table.draw();
