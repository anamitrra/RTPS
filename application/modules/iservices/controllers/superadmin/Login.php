...
class Login extends Frontend {
    ...
    public function password_login() {
        ...
        $this->form_validation->set_rules('email', 'Email', 'trim|required|xss_clean|strip_tags|max_length[255]');
        $this->form_validation->set_rules('password', 'Password', 'trim|required|xss_clean|strip_tags|max_length[255]');
        if ($this->form_validation->run() == FALSE) {
            ...
            $inputCaptcha = $this->input->post("admincaptcha");
            ...
            if($user) {
                if (verifyHashedPassword($password, $user->password)) {
                    $sessionArray = array(
                        ...
                        'name' => $user->name,
                        'office' => $user->office_address,
                        'district' => $user->district,
                        'isLoggedIn' => TRUE,
                    );
                    
                    $this->session->set_userdata($sessionArray);
                    redirect(base_url() . 'iservices/superadmin/dashboard');
                } else {
    ...
    public function loginMe() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            redirect(base_url() . 'iservices/superadmin/login');
        }

        if ($email == "" || $password == "") {
            $this->session->set_flashdata('error', 'Email or password cannot be empty');
            $this->index();
        } else {
...
//            return true;
    ...
    public function process_login() {
        ...
        if (!empty($this->session->userdata('redirectTo'))) {
            ...
            $status["url"] = base_url() . 'iservices/superadmin/dashboard';
        }
