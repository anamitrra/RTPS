...
class Vahan extends Rtps
{
    ...
    public function index($pur_cd)
    {
      ...
      if($guidelines){
        ...
        $data['action']=base_url('iservices/admin/v-procced');

        if($this->slug === "SA" || $this->slug === "PFC" || $this->slug === "CSC"){
          $data['apply_by']="PFC";
    ...
    public function acknowledgement(){
      ...
      $application_details=$this->intermediator_model->get_application_details(array("department_id"=>$app_ref_no,"pfc_payment_status"=>"Y"));
      if($application_details->service_id)
        $departmental_data=$this->portals_model->get_departmental_data($application_details->service_id);
      else
    ...
    public function retry($rtps_trans_id){
      if($rtps_trans_id){
        ...
        $transaction_details=$this->intermediator_model->get_transaction_detail($rtps_trans_id);
              ...
              $return_url=base_url("iservices/admin/get/vahan-response");
...
// var_dump($urlData);die;
    ...
    public function procced(){
      ...
      $this->form_validation->set_rules('pin_code', 'pin_code', 'required|trim');
        ...
        if ($this->form_validation->run() == FALSE) {
          $status["status"] = false;
          $status["error_msg"] = validation_errors();//"Please enter input for all require fields";
          return $this->output
            ->set_content_type('application/json')
            ->set_status_header(200)
      ...
      $user_id=$this->slug === "CSC" ? $this->session->userdata('userId'):$this->session->userdata('userId')->{'$id'};
      $portal_cd="AS";
      // $regn_no="AS01DB0032";
      // $chassi_no="11856";
    ...
    //  $redirect_url=base_url("request/create");
      ...
      $enc = $aes->encrypt();
   //   pre($enc);
      //check if url encryption is done
      if(empty($enc)){
        $status["status"] = false;
        $status["error_msg"] = "Something went wrong. Please try again";
        $status["ErrCode"] = "100";
        return $this->output
          ->set_content_type('application/json')
          ->set_status_header(200)
      ...
      $user_data=array(
        ...
        "applied_by"=> $this->slug === "CSC" ? $this->session->userdata('userId'):new ObjectId($this->session->userdata('userId')->{'$id'}),
