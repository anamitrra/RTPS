  ...
  */
  ...
  public function get_records() {
    ...
    if (!empty($records)) {
      ...
      foreach ($records as $rows) {
        $objId = $rows->{"_id"}->{'$id'};
        $btns = '<a href="' . base_url("iservices/admin/users/read/$objId") . '" data-toggle="tooltip" data-placement="top" title="Edit" class="" ><span class="fa fa-eye" aria-hidden="true"></span></a> ';
        $btns .= '<a href="' . base_url("iservices/admin/users/update/$objId") . '" data-toggle="tooltip" data-placement="top" title="Edit" class=""><span class="fa fa-edit" aria-hidden="true"></span></a>';
        $btns .= '<a href="' . base_url("iservices/admin/users/delete/$objId") . '" data-toggle="tooltip" data-placement="top" title="Delete" class="" onclick="return confirm(\'Are You Sure you want to delete?\')"><span class="fas fa-trash" aria-hidden="true"></span></a>';
        // $dept_obj=$this->department_model->get_services($rows->department[0]);
        $nestedData["userId"] = $sl;
        $nestedData["name"] = $rows->name;
        if($rows->isDeleted === '1' || $rows->isDeleted === 1){
          $nestedData["mobile"] = $rows->mobile."(Deactived)";
        }else{
  ...
  //  pre(strval($row->roleId));
    if ($row) {
      ...
      redirect(site_url('iservices/admin/users'));
    }
  }
  ...
  public function create() {
    ...
    $data = array(
      'button' => 'Create',
      'action' => base_url('iservices/admin/users/create_action'),
      'email' => set_value('email'),
      'password' => set_value('password'),
      'name' => set_value('name'),
      'registration_id' => set_value('registration_id'),
      ...
      'type_of_kiosk' => set_value('type_of_kiosk'),
  ...
  //  pre(validation_errors());
    if ($this->form_validation->run() == FALSE) {
      ...
      $data = array(
        'email' => $email,
        'password' => getHashedPassword($this->input->post('mobile', TRUE)),
        'registration_id' => $this->input->post('name', TRUE),
        'designation' => $this->input->post('designation', TRUE),
        'office_code' => "",//$this->input->post('office_code', TRUE),
        'dept_code' => "",//$this->input->post('dept_code', TRUE),
        'type_of_kiosk' => $this->input->post('type_of_kiosk', TRUE),
        'name' => $this->input->post('registration_id', TRUE),
        ...
        'district_id' => !empty($this->input->post('district_id', TRUE)) ? $this->input->post('district_id', TRUE) : '',
        'isDeleted' => '0',
...
//        'roles' => $this->input->post('roles', TRUE), // for multi role
    ...
    //  pre($data);
      ...
      redirect(base_url('iservices/admin/users'));
    }
  }
  ...
  public function update($id) {
    ...
    if ($row) {
      $data = array(
        ...
        'button' => 'Update',
        'action' => base_url('iservices/admin/users/update_action'),
        'userId' => $row->{"_id"}->{'$id'},
        'email' => $row->email,
        // 'password' => '123456789',
        'name' => $row->name,
        ...
        // 'office_code' => isset($row->office_code) ? $row->office_code : "",
        // 'dept_code' => isset($row->dept_code) ? $row->dept_code : "",
        'type_of_kiosk' => isset($row->type_of_kiosk) ? $row->type_of_kiosk : "",
        'registration_id' => isset($row->registration_id) ? $row->registration_id : "",
        'account1' => isset($row->account1) ? $row->account1 : "",
        'office_address' => $row->office_address,
        'mobile' => $row->mobile,
        'roles' => $this->roles_model->get_all([]),
...
//            'roles' => $this->input->post('roles', TRUE),
  ...
  public function delete($id) {
    ...
    if ($row) {
      $this->users_model->update($id, array('isDeleted'=>"1"));
  ...
  public function undo_delete() {
    $email=$_GET['email'];
    $row = $this->users_model->get_by_email($email);
    if ($row) {
      $this->users_model->update($row->{"_id"}->{'$id'}, array('isDeleted'=>"0"));
      $this->session->set_flashdata('message', 'Undo Delete Record Success');
      redirect(site_url('iservices/admin/users'));
    } else {
      $this->session->set_flashdata('message', 'Record Not Found');
      redirect(site_url('iservices/admin/users'));
  ...
  public function _rules() {
    ...
    $this->form_validation->set_rules('type_of_kiosk', 'Kiosk Type', 'trim|required');
  ...
  public function upload_excel() {
    ...
    public function upload_action(){
      ...
      //  pre(  $inputFileName);
        ...
        try{
          ...
          for ($row = 2; $row <= $highestRow; $row++){
              ...
              if(!empty($rowData[5])){
                $kioskType="";
                if($rowData[6] ==='RTPS Kiosk'){
                  $kioskType="RTPSKIOSK";
                }elseif($rowData[6] ==='RTPS-Kiosk'){
                  $kioskType="RTPSKIOSK";
                }elseif($rowData[6] ==='eDistrict' || $rowData[6] ==='eDistrict Kiosk'){
                  $kioskType="eDistrict";
                }
                ...
                $data = array(
                  ...
                  'mobile' => $mobile_list[0],
                  "alternative_mobile"=>  $mobile,
                  'registration_id' => $rowData[2],
                  'type_of_kiosk' => $kioskType,
      ...
      public function getdistricts(){
        $data = $this->users_model->get_districts( );
        if( $data){
