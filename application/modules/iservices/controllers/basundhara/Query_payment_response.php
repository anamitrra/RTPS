...
class Query_payment_response extends Frontend
{
  ...
  public function response()
  {
    ...
    $response = $_POST;
    $this->intermediator_model->update_row(array('query_department_id' => $DEPARTMENT_ID), array("query_payment_status" => $_POST['STATUS'], "query_payment_response" => $response));
    if ($_POST['STATUS'] === 'Y') {
      //check the grn for valid transactions
      if ($this->checkgrn($DEPARTMENT_ID, true) === $_POST['GRN']) {
        $this->intermediator_model->update_row(array('query_department_id' => $DEPARTMENT_ID), array("query_status" => "PR"));
     ...
     //   $this->update_payment_status( $DEPARTMENT_ID);
      ...
      //  echo "Something wrong in middle";
        $this->intermediator_model->update_row(array('query_department_id' => $DEPARTMENT_ID), array("query_payment_status" => "P", "query_payment_response.STATUS" => 'P'));
        $this->show_error();
      }
  ...
  public function checkgrn($DEPARTMENT_ID = null, $check = false,$return=false)
  ...
  public function cin_response()
  {
    if (!empty($_POST)) {
      if (!empty($_POST['DEPARTMENT_ID'])) {
        ...
        $this->intermediator_model->update_row(array('query_department_id' => $DEPARTMENT_ID), array(
  ...
  public function show_pay_ack($data)
  {
    ...
    $this->load->view('includes/frontend/header');
    $this->load->view('basundhara/pay_ack',$data);
    $this->load->view('includes/frontend/footer');
  }
