...
class Rtpspayment extends Rtps {
 /**
  * __construct
  *
  * @return void
  */
  ...
  public function payment($rtps_trans_id){ 
    ...
    $payment_details= (isset($transaction_data->payment_details) && is_array($transaction_data->payment_details))?$transaction_data->payment_details[0]:false;
    if($payment_details == false ){
          if($this->update_payment_config($rtps_trans_id) == false){
            $this->my_transactions();
          }else{
    ...
    if(property_exists($transaction_data,'applied_by') && !empty($transaction_data->applied_by)){
      if($this->is_admin()){
        ...
        $this->load->view('rtps_pfc_payment',$data);
     ...
     if (strpos($data['department_data']['PAN_NO'],"XXXXXX") !== false) {
          $data['department_data']['PAN_NO']="";
        }
  ...
  public function convenience_fee($rtps_trans_id){
    $financial_year=get_financial_year();
    if(empty($rtps_trans_id)){
      $this->my_transactions();
    }
    ...
    if(property_exists($transaction_data,'applied_by') && !empty($transaction_data->applied_by)){
      ...
      $portal_data=$this->portals_model->get_departmental_data($transaction_data->service_id);
      if(empty($portal_data)){
        $this->session->set_flashdata('errmessage', 'Something went wrong. Please try again');
        $this->my_transactions();
        return;
      }
      $additional_account=false;
      if($portal_data->portal_no === "16"){
        if( (isset($portal_data->ep_payment_account_code) && !empty($portal_data->ep_payment_account_code))){
          $additional_account=$portal_data->ep_payment_account_code;
        }else{
      ...
      $rtps_account=$this->config->item("rtps_account");
      $rtps_convenience_fee=$this->config->item("rtps_convenience_fee");
      $applicant_details=!empty($transaction_data->applicant_details) ? $transaction_data->applicant_details[0] : false;
      ...
      $data['department_data']=array(
        ...
        "SUB_SYSTEM"=>"ARTPS-SP|".base_url('iservices/transoprt_response/rtps_payment_response'),
        "PARTY_NAME"=>(isset($applicant_details->applicant_name) && strlen(trim($applicant_details->applicant_name))) ? $this->clean($applicant_details->applicant_name) : "RTPS TEAM", //"TEAM IGR", //applicant name , address of client
        "PIN_NO"=>!empty($applicant_details->pin_code) ? $applicant_details->pin_code : "781005",
        "ADDRESS1"=>!empty($applicant_details->address1) ? $applicant_details->address1 : "NIC",
        "ADDRESS2"=>!empty($applicant_details->address2) ? $applicant_details->address2 : "",
        "ADDRESS3"=>!empty($applicant_details->address3) ? $applicant_details->address3 : "781005",
        "MULTITRANSFER"=>"Y",
        "NON_TREASURY_PAYMENT_TYPE"=>"02",
      ...
      if( $additional_account){
        $data['department_data']['AC2_AMOUNT']=$transaction_data->amount;
        $data['department_data']['ACCOUNT2']=$additional_account;
        $data['department_data']['TOTAL_NON_TREASURY_AMOUNT']=floatval($rtps_convenience_fee) + floatval($transaction_data->amount);
      }
