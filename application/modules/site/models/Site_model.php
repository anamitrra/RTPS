...
class Site_model extends Mongo_model
{
    ...
    public function get_popular($mis_id_arr)
    {
        $this->mongo_db->select(array('service_name', 'seo_url'), array('_id'));
        // $filter = array('mis_id' => $mis_id);

        return  (array) $this->mongo_db->where_in('mis_id', $mis_id_arr)->get($this->table);
    }
    ...
    public function get_basundhara_services($lang = 'as')
    {
        return $this->mongo_db->aggregate($this->table, array(
            array(
                '$match' => ['service_id' => ['$in' => ["00014", "0005", "0006", "0007", "0008", "0009", "00010", "00011", "00012", "00013", "1410", "1207", "100011", "100012", "100013", "100014", "100015", "100016", "100017", "232", "231"]], 'online' => TRUE]
            ),
            array(
                '$lookup' => [
                    'from' => 'departments',
    ...
    public function get_services_by_categ_n_subcateg($cat, $subcat = null, $lang = 'as')
    {
        $filter = array(
            array(
                '$match' => ['cat_id' => intval($cat), 'online' => true]
            ),
            array(
                '$lookup' => [
                    'from' => 'departments',
        ...
        if (!empty($subcat)) {
            $filter[0]['$match']['sub_cat'] = trim($subcat);
        }

        // pre($filter);

        return $this->mongo_db->aggregate($this->table, $filter);
    }
    ...
    public function get_sub_categories($cat_id)
    {
        return $this->mongo_db->command(array(
            ...
            'filter' => ['cat_id' => intval($cat_id)],
            'projection' => ['_id' => 0, 'sub_categories' => 1],
        ));
    }

    public function get_services_from_sub($cat_id, $sub_cat)
    {
    ...
    public function update_sub($service_ob_id, $data)
    {
        return $this->update($service_ob_id, $data);
    }
}
