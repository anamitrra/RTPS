...
// pre(json_encode($kys->services_links ?? '', JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE));
// pre($settings);
?>
         ...
         <div class="py-3 text-start text-md-end kys-panel">
            ...
            <?php if (!empty($kys->service_url)) : ?>
               ...
               ?>
                  ...
                  <?php if ($kys->service_id == "0005") : ?>

                     <button type="button" data-apply-alt="<?= $kys->service_id ?>" class="btn rtps-btn-alt"> <?= $settings->apply_btn->$lang ?> </button>
...
<script>
   // Basundhara Field Mutation Service modification
   var kysAsk = "<?= $settings->kys_ask->$lang ?? ''  ?>";
   var serviceList = JSON.parse('<?= json_encode($kys->services_links ?? "", JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE)  ?>');
   var lang = "<?= $lang ?>";
