...
//            "pageTitle" => "Login"
            ...
            $this->load->view('admin/includes/frontend/header');
            $this->load->view('admin/login', $data);
            $this->load->view('admin/includes/frontend/footer');
        } else {
            redirect('site/admin/dashboard');
        }
    }
    ...
    public function loginMe()
    {
        ...
        if ($email == "" || $password == "") {
            ...
            if (isset($result) && $result != NULL) {
                ...
                redirect(base_url() . 'site/admin/dashboard');
            } else {
...
//            "pageTitle" => "Forgot Password"
    ...
    function resetPasswordUser()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->session->set_flashdata('error', validation_errors());
            redirect('dashboard/login/forgot-password');
        } else {
            ...
            if ($this->login_model->checkEmailExist($email)) {
                ...
                if ($save) {
                    $data1['reset_link'] = base_url("dashboard/reset-password/") . $data['activation_id'] . "/" . $encoded_email;
...
//        $email = base64_decode($email);
    ...
    function createPasswordUser()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->session->set_flashdata('error', validation_errors());
            redirect('dashboard/reset-password/'.$activation_id.'/'.base64_encode($email));
        } else {
...
//                $this->login_model->createPasswordUser($email, $password);
    ...
    {
        $this->load->model('admin/login_model');
        pre($this->login_model->createPasswordUser('site_manager@mail.in', 'Portal$admin@123'));
    }
