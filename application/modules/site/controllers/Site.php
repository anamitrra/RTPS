...
class Site extends Site_Controller
{
    ...
    public function index()
    {
        ...
        // Get categorywise services count
        $data['total_citizen'] = sizeof((array) $this->site_model->get_services_by_categ_n_subcateg(4, null, $this->lang));
        $data['total_business'] = sizeof((array) $this->site_model->get_services_by_categ_n_subcateg(8, null, $this->lang));
        $data['total_utility'] = sizeof((array) $this->site_model->get_services_by_categ_n_subcateg(15, null, $this->lang));

        $this->render_view_new('index', $data);
    }
    ...
    public function faq()
    {
        ...
        $this->mongo_db->where([]);
        $data['faq']->content = (array) $this->mongo_db->get('faq_chat');
    ...
    // Get Pupular services
    public function get_popular()
    {

        $mis_ids_arr = explode(',', $_GET['mis_id'] ?? '');
        // pre($mis_ids_arr); 
        // $service_id = json_decode($this->input->get('mis_id', true));

        if (empty($mis_ids_arr)) {
            $data['details'] = [];
        } else {
            $data['details'] = $this->site_model->get_popular($mis_ids_arr);
        }

        return $this->output
            ->set_content_type('application/json')
            ->set_status_header(200)
            ->set_output(json_encode($data['details'], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE));
    }

    public function all_policies()
    {
        $data['data'] = $this->settings_model->get_settings('policies');
        // pre($data);
        $this->render_view_new('policies', $data);
    }

    public function dsc()
    {
        $data['data'] = $this->settings_model->get_settings('dsc');
        // pre($data);
        $this->render_view_new('dsc', $data);
    }

    public function categorywise_services_api()
    {
        // Get categorywise services count
        $data['data']['total_citizen'] = sizeof((array) $this->site_model->get_services_by_categ_n_subcateg(4, null, 'en'));
        $data['data']['total_business'] = sizeof((array) $this->site_model->get_services_by_categ_n_subcateg(8, null, 'en'));
        $data['data']['total_utility'] = sizeof((array) $this->site_model->get_services_by_categ_n_subcateg(15, null, 'en'));
