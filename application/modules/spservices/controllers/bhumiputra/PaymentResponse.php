...
class PaymentResponse extends Frontend
{
    ...
    public function response()
    {
        ...
        $response = $_POST;
        $this->caste_model->update_where(['form_data.department_id' => $DEPARTMENT_ID], array("form_data.payment_status" => $_POST['STATUS'], "form_data.pfc_payment_response" => $response));
        if ($_POST['STATUS'] === 'Y') {
            if ($this->checkgrn($DEPARTMENT_ID, true) === $_POST['GRN']) {
                //if (true) {
                // $dbrow = $this->registration_model->get_row(array('form_data.department_id' => $DEPARTMENT_ID));
                ...
                // );
                // $this->registration_model->update_where(['department_id' => $DEPARTMENT_ID], $data);
                ...
                $transaction_data = $this->caste_model->get_row(array('form_data.department_id' => $DEPARTMENT_ID));
                if ($transaction_data->service_data->service_id === "CASTE") {
                    $ref = modules::load('spservices/bhumiputra/registration');
                    $ref->finalsubmition($transaction_data->_id->{'$id'}, true);
                    redirect(base_url('spservices/bhumiputra/applications/acknowledgement/') . $transaction_data->_id->{'$id'});
                }
                ...
                $this->caste_model->update_where(['department_id' => $DEPARTMENT_ID], ["status" => "PAYMENT_COMPLETED", "payment_status" => "PAYMENT_FAILED"]);
                $this->show_error();
            } //End of if else
        } else {
            $this->show_error();
    ...
    public function cin_response()
    {
        if (!empty($_POST)) {
            if (!empty($_POST['DEPARTMENT_ID'])) {
                ...
                $this->caste_model->update_row(array('department_id' => $DEPARTMENT_ID), array(
                ...
                if ($STATUS === 'Y') {
                    $transaction_data = $this->caste_model->get_row(array('department_id' => $DEPARTMENT_ID));
