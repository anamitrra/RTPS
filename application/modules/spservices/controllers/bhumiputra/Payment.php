...
class Payment extends Rtps
{
    ...
    private $serviceName = "Caste Certificate";
    ...
    public function verify($obj_id = null)
    {
        ...
        if ($obj_id) {
            ...
            $application = $this->caste_model->get_row($filter);
    ...
    //         $payData = array(
    //             'form_data.aadhaar_verify_status' => 1,
    //             'form_data.status' => 'PAYMENT_INITIATED',
    //             'form_data.payment_status' => 'START'
    //         );
    //         $this->registration_model->update_where(['_id' => new ObjectId($obj_id)], $payData); //Update status    
    ...
    public function initiate($obj_id = null)
    {
        ...
        if ($obj_id) {
            ...
            $this->caste_model->update_where($filter, array('service_data.appl_status' => 'payment_initiated'));
            //pre($filter);
            ...
            if ($this->slug !== "user") {
                ...
                $this->load->view('includes/frontend/header');
                $this->load->view('bhumiputra/kiosk_payment', $data);
                $this->load->view('includes/frontend/footer');
            } else {
                $this->payment_make($obj_id);
            }
    ...
    private function payment_make($obj_id = null)
    {
        ...
        if (count((array) $dbRow)) {
            ...
            // if (property_exists($dbRow, "service_data.department_id") && property_exists($dbRow, "payment_params")) {
            //     $this->check_payment_status($dbRow->department_id);
            // }
            ...
            if ($this->slug === "user") {
                ...
                $data['department_data'] = array(
                    ...
                    "SUB_SYSTEM" => "ARTPS-SP|" . base_url('spservices/bhumiputra/PaymentResponse/response'),
                    "PARTY_NAME" => $dbRow->form_data->applicant_name ?? 'RTPS TEAM',
                    "PIN_NO" => $dbRow->form_data->resPinCode ?? '781005',
                    "ADDRESS1" => $dbRow->form_data->resVillageTown ?? 'NIC',
                    "ADDRESS2" => $dbRow->form_data->resPostOffice ?? 'TEAM',
                    "ADDRESS3" => $dbRow->form_data->resDistrict ?? 'Kamrup',
                    "MULTITRANSFER" => "Y",
                    "NON_TREASURY_PAYMENT_TYPE" => "02",
                    "TOTAL_NON_TREASURY_AMOUNT" => "30",
                    "AC1_AMOUNT" => "30",
                    "ACCOUNT1" => $districtRow->gras_account_code, //$user->account1,
                );
                //pre($data);
            } else {
                if ($this->slug === "CSC") {
                ...
                if ($this->session->userdata('role') === "PFC") {
                ...
                $data['department_data'] = array(
                    ...
                    "MULTITRANSFER" => "Y",
                    "NON_TREASURY_PAYMENT_TYPE" => "02",
                    "TOTAL_NON_TREASURY_AMOUNT" => $total_amount + 10,
                    "AC1_AMOUNT" => "10",
                    "ACCOUNT1" => $districtRow->gras_account_code, //$user->account1,
            ...
            //pre($res);
            if ($res) {
                $this->load->view('spservices/bhumiputra/payment', $data);
            } else {
    ...
    public function payment_update($data)
    {
       ...
       // pre($data); 
        ...
        //pre($rtps_trans_id, $data_to_update);
        $result = $this->caste_model->update_payment_status($rtps_trans_id, $data_to_update);
        //pre ($result);
