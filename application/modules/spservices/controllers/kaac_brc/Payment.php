...
class Payment extends Rtps
{
    ...
    public function application_submission($obj_id, $service_id)
    {
        ...
        if(in_array($service_id, $service_ids)){ 
            $ref = modules::load('spservices/kaac_brc/registration');
    ...
    private function check_pfc_payment_status($DEPARTMENT_ID = null)
    {
        if ($DEPARTMENT_ID) {
            ...
            $ref = modules::load('spservices/kaac_brc/Payment_response');
    ...
    public function initiate($obj_id = null)
    {
        ...
        if (count((array) $dbRow)) {
            ...
            $this->kaac_registration_model->update_where(['_id' => new ObjectId($obj_id)], array('service_data.appl_status' => 'payment_initiated')); //Update status            
            ...
            if (!empty($this->session->userdata('role'))) {
                ...
                $this->load->view('kaac/sec_group/kiosk_payment', $data);
        ...
        if (count((array) $dbRow)) {
            ...
            if ($this->slug === "user") {
                $data['department_data'] = array(
                    ...
                    "SUB_SYSTEM" => "ARTPS-SP|" . base_url('spservices/kaac_brc/Payment_response/response'),

                    
                    "PARTY_NAME" => $dbRow->form_data->applicant_name ?? 'RTPS TEAM',
                    "PIN_NO" => $dbRow->form_data->pin_code ?? '781005',
                    "ADDRESS1" => $dbRow->form_data->police_station ?? 'NIC',
                    ...
                    "ACCOUNT1" => $this->config->item('rtps_convenience_fee_account'), //$dbRowGras->ACCOUNT1, 
                );
                ...
                if ($this->slug === "CSC") {
                    ...
                    $data['department_data']['ACCOUNT1'] = $this->convenience_fee_account;
                }
            }
