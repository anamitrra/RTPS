...
class Payment_response extends Frontend
{
  ...
  public function response()
  {
    ...
    $response = $_POST;
    $this->umang_model->update_row(array('form_data.department_id' => $DEPARTMENT_ID), array("form_data.pfc_payment_status" => $_POST['STATUS'], "form_data.pfc_payment_response" => $response));
    if ($_POST['STATUS'] === 'Y') {
      if ($this->checkgrn($DEPARTMENT_ID, true) === $_POST['GRN']) {
        $transaction_data = $this->umang_model->get_row(array('form_data.department_id' => $DEPARTMENT_ID));
        $ref = modules::load('spservices/umang/payment');
        $ref->callBack($transaction_data->_id->{'$id'});
        return;
      } else {
        $this->umang_model->update_row(array('form_data.department_id' => $DEPARTMENT_ID), array("form_data.pfc_payment_status" => "P", "form_data.pfc_payment_response.STATUS" => 'P'));
        $this->show_error();
      }
    } else {
      $this->show_error();
  ...
  public function cin_response()
  {
    if (!empty($_POST)) {
      if (!empty($_POST['DEPARTMENT_ID'])) {
        ...
        $this->umang_model->update_row(array('form_data.department_id' => $DEPARTMENT_ID), array(
        ...
        if ($STATUS === 'N') {
          redirect(base_url('spservices/umang/payment/verify/' . $transaction_data->_id->{'$id'}));
        }
      }
    }
