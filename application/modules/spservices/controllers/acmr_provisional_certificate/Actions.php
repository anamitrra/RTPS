...
class Actions extends Upms
{
  ...
  public function generate_certificate($appl_no=null)
  {
    ...
    $html = $this->load->view('acmr_provisional_certificate/output_certificate', array('data' => $pdata,'qr' => $qr_file_name,'certificate_no' => $certificate_no), true);
   
    $this->load->library('dpdf');
    $pdf_path = $this->dpdf->createPDF($html, $filename, 'A4', 'portrait', 'ACMR/PC' .$pdata[0]->service_data->service_id, true);
    return $pdf_path;
  }
    ...
    public function extractRefNo($string)
  {
    ...
    if (isset($matches[1])) {
      // $str = "AS/".date('Y'). "/"."PRV/ACMR". "/" . $matches[1];
      $str = $matches[1];
      return $str;
    } else {
      return null;
