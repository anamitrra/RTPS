...
class Payment_query_response extends Frontend
{
    ...
    public function response()
    {
        ...
        $this->registration_model->update_row(array('form_data.query_department_id' => $DEPARTMENT_ID), array("form_data.query_payment_status" => $_POST['STATUS'], "form_data.query_payment_response" => $response));
        if ($_POST['STATUS'] === 'Y') {
          
            //check the grn for valid transactions
            if ($this->checkgrn($DEPARTMENT_ID, true) === $_POST['GRN']) {
                //$this->checkgrn($DEPARTMENT_ID, true) === $_POST['GRN']
               // $this->registration_model->update_row(array('form_data.query_department_id' => $DEPARTMENT_ID), array("processing_history.status" => "QA"));
                $transaction_data = $this->registration_model->get_row(array('form_data.query_department_id' => $DEPARTMENT_ID));
                if($transaction_data->service_data->service_id === $this->serviceId){
                    ...
                    $processing_history = $transaction_data->processing_history??array();
                    $processing_history[] = array(
                        "processed_by" => "Payment Query submitted by ".$transaction_data->form_data->applicant_name,
                        "action_taken" => "Payment Query submitted",
                        "remarks" => "Payment Query submitted by ".$transaction_data->form_data->applicant_name,
...
//pre($dataApp);
                    $this->registration_model->update_row(array('form_data.query_department_id' => $DEPARTMENT_ID), array("status" => "QUERY_ANSWERED"));
                    $this->registration_model->update_row(array('form_data.query_department_id' => $DEPARTMENT_ID), $dataApp);
                    //$payment_type = "bp";
                    //$ref->submit_to_backend($transaction_data->_id->{'$id'});
                   // echo "payment successful. Please add payment ack page";
                    redirect(base_url('spservices/acmr_provisional_certificate/registration/payment_ack/') . $transaction_data->_id->{'$id'});
                }
            } else {
                //grn does not match Something went wrong
                echo "Something wrong in middle";
                $this->registration_model->update_row(array('form_data.query_department_id' => $DEPARTMENT_ID), array("form_data.query_payment_status" => "P", "form_data.query_payment_response.STATUS" => 'P'));
                $this->show_error();
            }
    ...
    public function cin_response()
    {
        ...
        if (!empty($_POST)) {
            if (!empty($_POST['DEPARTMENT_ID'])) {
                ...
                $this->registration_model->update_row(array('form_data.query_department_id' => $DEPARTMENT_ID), array(
