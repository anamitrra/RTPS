...
class Payment extends Rtps {
    ...
    private function check_pfc_payment_status($DEPARTMENT_ID = null) {
        if ($DEPARTMENT_ID) {
            ...
            $ref = modules::load('spservices/marriageregistration_landhub/paymentresponse');
    ...
    public function application_submission($obj_id) {
        $dbrow = $this->marriageregistrations_model->get_row(['_id' => new ObjectId($obj_id)]);
        $paymentresponse = modules::load('spservices/marriageregistration_landhub/paymentresponse');
        $paymentresponse->post_data($dbrow->department_id);
    ...
    public function initiate($obj_id = null) {
        ...
        if ($dbRow) {
            ...
            $this->marriageregistrations_model->update_where(['_id' => new ObjectId($obj_id)], $payData); //Update status            
            ...
            if (!empty($this->session->userdata('role'))) {
                ...
                $this->load->view('marriageregistration_landhub/payment_view', $data);
    ...
    private function payment_make($obj_id = null) {
        $dbRow = $this->marriageregistrations_model->get_by_doc_id($obj_id); //pre($dbRow);
        if (count((array) $dbRow)) {
            $applicant_name = $this->clean($dbRow->applicant_first_name);
            if(isset($dbRow->applicant_middle_name) && strlen($dbRow->applicant_middle_name)) {
                $applicant_name = $applicant_name.' '.$this->clean($dbRow->applicant_middle_name);
            }
            if(isset($dbRow->applicant_last_name) && strlen($dbRow->applicant_last_name)) {
                $applicant_name = $applicant_name.' '.$this->clean($dbRow->applicant_last_name);
            }
            ...
            if ($sroRow) {
                ...
                if (strlen($treasuryCode) && strlen($officeCode)) {
                    ...
                    if ($serviceRow) {
                        ...
                        $noticeFee = 50;
                        $userCharge = 200;
                        $CHALLAN_AMOUNT = (float) $noticeFee + (float) $userCharge;
                        $curretnYear = date('Y');
                        $data['department_data'] = array(
                            ...
                            "ADDRESS1" => (isset($dbRow->bride_present_address1) && strlen($dbRow->bride_present_address1)) ? $dbRow->bride_present_address1 : "NIC",
                            "ADDRESS2" => isset($dbRow->bride_present_address2) ? $dbRow->bride_present_address2 : "",
                            "ADDRESS3" => isset($dbRow->bride_permanent_pin) ? $dbRow->bride_permanent_pin : "781005",
                            "MOBILE_NO" => $dbRow->applicant_mobile_number,
                            "DEPARTMENT_ID" => $DEPARTMENT_ID,
                            // "SUB_SYSTEM"=>"REV-SP|".base_url('iservices/admin/get/payment-response'),
                            "SUB_SYSTEM" => "ARTPS-SP|" . base_url('spservices/marriageregistration_landhub/paymentresponse/paymentmade'),
                        );
    ...
    public function update_pfc_payment_amount($data, $CHALLAN_AMOUNT) {
        ...
        $result = $this->marriageregistrations_model->update_row(['rtps_trans_id' => $rtps_trans_id], $data_to_update);
