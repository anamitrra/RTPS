...
class Payment_response extends Frontend
{
    ...
    public function response()
    {
        ...
        $this->ahsec_correction_model->update_row(array('form_data.department_id' => $DEPARTMENT_ID), array("form_data.pfc_payment_status" => $_POST['STATUS'], "form_data.pfc_payment_response" => $response));
        if ($_POST['STATUS'] === 'Y') { 
            //check the grn for valid transactions 
            if ($this->checkgrn($DEPARTMENT_ID, true) === $_POST['GRN']) { 
                $transaction_data = $this->ahsec_correction_model->get_row(array('form_data.department_id' => $DEPARTMENT_ID));
                $service_ids = array("AHSECCRC", "AHSECCADM", "AHSECCMRK", "AHSECCPC");
                if(in_array($transaction_data->service_data->service_id, $service_ids)){        
                ...
                $this->ahsec_correction_model->update_row(array('form_data.department_id' => $DEPARTMENT_ID), array("form_data.pfc_payment_status" => "P", "form_data.pfc_payment_response.STATUS" => 'P'));
                $this->show_error();
            }
    ...
    { // TODO: need to check which are params to update
        ...
        if ($DEPARTMENT_ID) {
            ...
            if ($res) {
                ...
                //if($STATUS === "Y"){
                $this->ahsec_correction_model->update_row(
    ...
    public function cin_response()
    {
        ...
        if (!empty($_POST)) {
            if (!empty($_POST['DEPARTMENT_ID'])) {
                ...
                $this->ahsec_correction_model->update_row(array('form_data.department_id' => $DEPARTMENT_ID), array(
