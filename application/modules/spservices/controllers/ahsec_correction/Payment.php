...
class Payment extends Rtps
{
    ...
    public function __construct()
    {
        ...
        //$this->load->model('ahsec_correction/gras_config_model');
    ...
    {
        $service_ids = array("AHSECCRC", "AHSECCADM", "AHSECCMRK", "AHSECCPC");
        if(in_array($service_id, $service_ids)){ 
            $ref = modules::load('spservices/ahsec_correction/ahseccor');
    ...
    private function check_pfc_payment_status($DEPARTMENT_ID = null)
    {
        if ($DEPARTMENT_ID) {
            ...
            $ref = modules::load('spservices/ahsec_correction/Payment_response');
    ...
    public function initiate($obj_id = null)
    {
        ...
        $dbRow = $this->ahsec_correction_model->get_by_doc_id($obj_id);
        if (count((array) $dbRow)) {
            ...
            $this->ahsec_correction_model->update_where(['_id' => new ObjectId($obj_id)], array('service_data.appl_status' => 'payment_initiated')); //Update status            
            $data = array("pageTitle" => "Make Payment");
            $data["dbrow"] = $dbRow;

            if (!empty($this->session->userdata('role'))) {
                $data['edistrict_amtron_charge'] = 200;
                ...
                $this->load->view('ahsec_correction/kiosk_payment', $data);
    ...
    private function payment_make($obj_id = null)
    {
        ...
        if (count((array) $dbRow)) {
            // $dbRowGras = $this->gras_config_model->get_row(array("eDISTRICT_REVENUE_CIRCLE" => $dbRow->form_data->revenuecircle));

            // if (empty($dbRowGras)) {
            //     $this->session->set_flashdata('errmessage', 'Circle office (' . $dbRow->form_data->revenuecircle . ') is not activated for payment!');
            //     $this->my_transactions();
            // } else if (empty($dbRowGras->OFFICE_CODE)) {
            //     $this->session->set_flashdata('errmessage', 'Circle office (' . $dbRowGras->eDISTRICT_REVENUE_CIRCLE . ') is not activated for payment!');
            //     $this->my_transactions();
            // }
            ...
            if ($this->slug === "user") {
                $application_charge = 200;
                $data['department_data'] = array(
                    "DEPT_CODE" => "ARI", //$dbRowGras->DEPT_CODE,
                    "OFFICE_CODE" => "ARI000", //$dbRowGras->OFFICE_CODE,
                    ...
                    "SUB_SYSTEM" => "ARTPS-SP|" . base_url('spservices/ahsec_correction/Payment_response/response'),
                    ...
                    "MULTITRANSFER" => "Y",
                    "NON_TREASURY_PAYMENT_TYPE" => "01", //$dbRowGras->NON_TREASURY_PAYMENT_TYPE,
                    "TOTAL_NON_TREASURY_AMOUNT" => $application_charge + $this->convenience_fee,
                    "AC1_AMOUNT" => $application_charge,
                    "ACCOUNT1" => "ARI68751", //$dbRowGras->ACCOUNT1, 
                ...
                if ($this->slug === "CSC") {
                    ...
                    $account = $user->account1; // Actual Account
                    //$account = "PFC23362"; //For Test Only
                    // $mobile = $user->mobile;
                ...
                if ($this->slug === "CSC") {
                    $data['department_data']['TOTAL_NON_TREASURY_AMOUNT'] = $total_amount + $application_charge + $this->convenience_fee;
                    $data['department_data']['AC1_AMOUNT'] = $application_charge;
                    $data['department_data']['ACCOUNT1'] = "ARI68751"; //$dbRowGras->ACCOUNT1;
                    $data['department_data']['AC2_AMOUNT'] = $total_amount;
                    $data['department_data']['ACCOUNT2'] = $account;
                    $data['department_data']['AC3_AMOUNT'] = $this->convenience_fee;
                    ...
                    if ($user->type_of_kiosk == "eDistrict") {
                        ...
                        $data['department_data']['AC1_AMOUNT'] = $application_charge;
                        $data['department_data']['ACCOUNT1'] = "ARI68751"; //$dbRowGras->ACCOUNT1;
                        $data['department_data']['AC2_AMOUNT'] = $total_amount + $this->convenience_fee;
                        $data['department_data']['ACCOUNT2'] = $account;
                    }
                }
            }
    ...
    public function update_pfc_payment_amount($data, $application_charge)
    {
        ...
        // pre($data_to_update);
        $result = $this->ahsec_correction_model->update_payment_status($appl_ref_no, $data_to_update);
    ...
    public function verify($obj_id = null)
    {
        ...
        if ($obj_id) {
            ...
            $application = $this->ahsec_correction_model->get_row($filter);
