...
class Ahseccor extends Rtps
{
    ...
    public function searchdetails_submit()
    {
        ...
        if ($this->serviceId != "AHSECCRC") {
            $this->form_validation->set_rules('ahsec_yearofappearing', 'Year of appearing/passing in HS Final Examination', 'trim|required|xss_clean|strip_tags');

            $lower_limit_yr = "2006";
            $upper_limit_yr = "2017";
            if (!empty($this->input->post("ahsec_yearofappearing"))) {
                if (($this->input->post("ahsec_yearofappearing") >= $lower_limit_yr) && ($this->input->post("ahsec_yearofappearing") <= $upper_limit_yr)) {
                ...
                $this->form_validation->set_rules('ahsec_yearofappearing', 'AHSEC Year of appearing', 'trim|required');
            }
        } else {
            if ($this->serviceId == "AHSECCRC") {
                if ((!empty($this->input->post("ahsec_yearofappearing"))) && ($this->input->post("ahsec_yearofappearing") != "Not yet appeared")) {
                    $this->form_validation->set_rules('ahsec_admit_roll', 'AHSEC admit roll', 'trim|required|xss_clean|strip_tags');
                    $this->form_validation->set_rules('ahsec_admit_no', 'AHSEC admit no', 'trim|required|xss_clean|strip_tags');
                }
                $this->form_validation->set_rules('ahsec_yearofappearing', 'AHSEC Year of appearing', 'trim|required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $limit_yr_exam = "2018";
            if ((!empty($this->input->post("ahsec_admit_roll"))) && (!empty($this->input->post("ahsec_admit_no"))) && (!empty($this->input->post("ahsec_yearofappearing")))) {
                ...
                if ($this->input->post("ahsec_yearofappearing") >= $limit_yr_exam) {
            ...
            if ($this->serviceId != "AHSECCRC") {
                ...
                $filter45["form_data.ahsec_yearofappearing"] = (int) $this->input->post("ahsec_yearofappearing");
            }
            //

            /// An applicant can apply for any correction service maximum twice.
            $dbrow_data45_cnt = $this->ahsec_correction_model->rows_count($filter45);
            if ($dbrow_data45_cnt > 3) {
                $this->session->set_flashdata('error', 'You can apply for 3 times only. !!');
                redirect('spservices/ahsec_correction/ahseccor/searchdetails/' . $this->serviceId);
            }
            //


            //can’t apply for the same service again until his earlierapplication for the service is delivered or rejected.
            $dbrow_data45 = $this->ahsec_correction_model->get_rows($filter45);
            $dbrow_cnt_data45 = $this->ahsec_correction_model->rows_count($filter45);
            ...
            $form_data = [
                ...
                'p_comp_permanent_address' => $dbrow_data->form_data->p_comp_permanent_address ?? "",
                'p_state' => $dbrow_data->form_data->p_state ?? "",
                'p_district' => $dbrow_data->form_data->p_district ?? "",
                'p_pin_code' => $dbrow_data->form_data->p_pin_code ?? "",

                'c_comp_permanent_address' => $dbrow_data->form_data->c_comp_permanent_address ?? "",
                'c_state' => $dbrow_data->form_data->c_state ?? "",
                'c_district' => $dbrow_data->form_data->c_district ?? "",
                'c_pin_code' => $dbrow_data->form_data->c_pin_code ?? "",
                ...
                'ahsec_admit_roll' => ($this->serviceId != "AHSECCRC") ? ($ahsecmarksheet_data->Roll ?? (int) $this->input->post("ahsec_admit_roll")) : '',
                'ahsec_admit_no' => ($this->serviceId != "AHSECCRC") ? ($ahsecmarksheet_data->No ?? (int) $this->input->post("ahsec_admit_no")) : '',
                'ahsec_yearofappearing' => ($this->serviceId != "AHSECCRC") ? ($ahsecmarksheet_data->Year_of_Examination ?? $this->input->post("ahsec_yearofappearing")) : '',
    ...
    {
        if ($this->checkObjectId($objId)) {
            $dbRow = $this->ahsec_correction_model->get_row(array("_id" => new ObjectId($objId), "service_data.appl_status" => "D"));
            if ($dbRow) {
                $data = array(
                    "dbrow" => $dbRow,
                );
                ...
                $this->load->view('includes/frontend/header');
                $this->load->view('ahsec_correction/duplicate_cert_recorrection_view', $data);
                $this->load->view('includes/frontend/footer');
            } else {
                $this->session->set_flashdata('error', 'No records found in query mode against object id : ' . $objId);
                $this->my_transactions();
    ...
    public function request_for_recorrection_submit()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (count((array) $dbRow)) {
                ...
                redirect('spservices/ahsec_correction/ahseccor/applicationpreview/' . $objId);
