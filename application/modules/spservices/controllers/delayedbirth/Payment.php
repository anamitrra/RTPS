...
class Payment extends Rtps
{
    ...
    public function application_submission($obj_id, $service_id)
    {
        if ($service_id === $this->serviceId) {
            $ref = modules::load('spservices/delayedbirth/pdbr');
    ...
    private function check_pfc_payment_status($DEPARTMENT_ID = null)
    {
        if ($DEPARTMENT_ID) {
            ...
            $ref = modules::load('spservices/delayedbirth/Payment_response');
            $grndata = $ref->checkgrn($DEPARTMENT_ID, false, true);

            if (!empty($grndata)) {
                if ($grndata['STATUS'] === 'Y') {
                    $this->session->set_flashdata('errmessage', 'Payment mode is already in Y');
                    $this->my_transactions();
                    return true;
                }
                $ar = array('N', 'A');
                if (!empty($grndata['GRN']) && !in_array($grndata['STATUS'], $ar)) {
                    //$this->session->set_flashdata('errmessage', 'Payment status is either in N or A');
                    $this->session->set_flashdata('errmessage', 'Payment status is not updated. please retry after sometime.!');
                    $this->my_transactions();
                    return;
                }
            }
    ...
    public function initiate($obj_id = null)
    {
        ...
        $dbRow = $this->delayed_birth_model->get_by_doc_id($obj_id);
        if (count((array) $dbRow)) {
            ...
            if (property_exists($dbRow->form_data, 'pfc_payment_status') && $dbRow->form_data->pfc_payment_status == 'Y') {
                ...
                $this->session->set_flashdata('errmessage', 'Payment already and hence the status has updated');
                $this->my_transactions();
            }
            ...
            $this->delayed_birth_model->update_where(['_id' => new ObjectId($obj_id)], array('service_data.appl_status' => 'payment_initiated')); //Update status            
            ...
            if (!empty($this->session->userdata('role'))) {
                ...
                $this->load->view('delayedbirth/kiosk_payment', $data);
        ...
        if (count((array) $dbRow)) {
            ...
            if ($this->slug === "user") {
                $data['department_data'] = array(
                    ...
                    "SUB_SYSTEM" => "ARTPS-SP|" . base_url('spservices/delayedbirth/Payment_response/response'),
            ...
            if ($res) {
                ...
                $this->session->set_flashdata('errmessage', 'Error in payment status updating');
                $this->my_transactions();
            } //End of if else
        } else {
            $this->session->set_flashdata('errmessage', 'No records found');
    ...
    public function update_pfc_payment_amount($data)
    {
        ...
        $result = $this->delayed_birth_model->update_payment_status($appl_ref_no, $data_to_update);
    ...
    public function verify($obj_id = null)
    {
        ...
        if ($obj_id) {
            ...
            $application = $this->delayed_birth_model->get_row($filter);
