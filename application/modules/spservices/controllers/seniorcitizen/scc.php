...
class Scc extends Rtps
{
    ...
    public function index($obj_id = null)
    {
        ...
        $data = array("pageTitle" => "Application for Senior Citizen Certificate");
        $filter = array("_id" => new ObjectId($obj_id), "service_data.appl_status" => "DRAFT");
        $data["dbrow"] = $this->senior_citizen_model->get_row($filter);
        $data['usser_type'] = $this->slug;
        $data["sro_dist_list"] = $this->senior_citizen_model->sro_dist_list();

        $this->load->view('includes/frontend/header');
        $this->load->view('seniorcitizen/senior_citizen', $data);
    ...
    public function getlocation()
    {
        $id = $_GET['id'];
        if ($id) {
            $data = $this->senior_citizen_model->get_sro_list($id);
            if ($data) {
    ...
    public function submit()
    {
        ...
        $this->form_validation->set_rules('spouse_name', 'Spouse name', 'trim|xss_clean|strip_tags|regex_match[/^[a-zA-Z ]*$/]|max_length[255]');
        ...
        $this->form_validation->set_rules('pan_no', 'PAN no', 'trim|required|xss_clean|strip_tags|regex_match[/^([a-zA-Z]){5}([0-9]){4}([a-zA-Z]){1}?$/]');
        ...
        $this->form_validation->set_rules('address_line2', 'Address line 2', 'trim|required|xss_clean|strip_tags');
        ...
        $this->form_validation->set_rules('landline_no', 'Land line no', 'trim|integer|exact_length[10]|xss_clean|strip_tags');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            while (1) {
                ...
                $rows = $this->senior_citizen_model->get_row($filter);
            ...
            $form_data = [
                ...
                'allowance_details' => trim($this->input->post("allowance_details")),
            ...
            if (strlen($objId)) {
                $this->senior_citizen_model->update_where(['_id' => new ObjectId($objId)], $inputs);
                $this->session->set_flashdata('success', 'Your application has been successfully submitted');
                redirect('spservices/senior-citizen-certificate/fileuploads/' . $objId);
            } else {
                $insert = $this->senior_citizen_model->insert($inputs);
                if ($insert) {
                    $objectId = $insert['_id']->{'$id'};
                    $this->session->set_flashdata('success', 'Your application has been successfully submitted');
                    redirect('spservices/senior-citizen-certificate/fileuploads/' . $objectId);
                    exit();
                } else {
    ...
    {
        $dbRow = $this->senior_citizen_model->get_by_doc_id($objId);
        if (count((array)$dbRow)) {
            ...
            redirect('spservices/seniorcitizen/scc/');
        } //End of if else
    } //End of fileuploads()
    ...
    public function finalsubmition($obj = null)
    {
        ...
        if ($obj) {
            $dbRow = $this->senior_citizen_model->get_by_doc_id($obj);
            if ($dbRow->service_data->appl_status == "payment_initiated" && !empty($dbRow->form_data->pfc_payment_response->GRN) && ($dbRow->form_data->pfc_payment_response->STATUS === "Y")) {
                ...
                $myfile = fopen("D:\\TESTDATA\\SCC_DGL.txt", "a") or die("Unable to open file!");
    ...
    public function submitfiles_old()
    {
        ...
        if(empty($uploadedFiles['passport_photo_old']) || empty($uploadedFiles['proof_of_retirement_old'])|| empty($uploadedFiles['age_proof_old'])|| empty($uploadedFiles['address_proof_old']))
        {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
                ...
                'form_data.proof_of_retirement' => strlen($proof_of_retirement) ? $proof_of_retirement : $this->input->post("proof_of_retirement_old"),
            ...
            $this->senior_citizen_model->update_where(['_id' => new ObjectId($objId)], $data);
            //$this->session->set_flashdata('success', 'Your application has been successfully submitted');
            redirect('spservices/seniorcitizen/scc/preview/' . $objId);
        } //End of if else
    } //End of submitfiles()
    ...
    public function submitfiles()
    {
        ...
        if (empty($this->input->post("proof_of_retirement_old"))) {
            if ((empty($this->input->post("proof_of_retirement_temp"))) && (($_FILES['proof_of_retirement']['name'] == ""))) {
                $this->form_validation->set_rules('proof_of_retirement', 'Proof of retirement document', 'required');
            }
        }

        if (empty($this->input->post("age_proof_old"))) {
            if ((empty($this->input->post("age_proof_temp"))) && (($_FILES['age_proof']['name'] == ""))) {
                $this->form_validation->set_rules('age_proof', 'Age proof document', 'required');
            }
        }
        ...
        $proof_of_retirement = "";
        if ($_FILES['proof_of_retirement']['name'] != "") {
            $proofofRetirement = cifileupload("proof_of_retirement");
            $proof_of_retirement = $proofofRetirement["upload_status"] ? $proofofRetirement["uploaded_path"] : null;
        }
        if (!empty($this->input->post("proof_of_retirement_temp"))) {
            $proofofRetirement = movedigilockerfile($this->input->post('proof_of_retirement_temp'));
            $proof_of_retirement = $proofofRetirement["upload_status"] ? $proofofRetirement["uploaded_path"] : null;
        }
        ...
        // pre($uploadedFiles['passport_photo_old']);
    ...
    public function track($objId = null)
    {
        ...
        if (count((array)$dbRow)) {
            ...
            $this->load->view('includes/frontend/header');
            $this->load->view('seniorcitizen/sccertificatetrack_view', $data);
            $this->load->view('includes/frontend/footer');
        } else {
            $this->session->set_flashdata('error', 'No records found against object id : ' . $objId);
            redirect('spservices/seniorcitizen/scc');
    ...
    public function queryform($objId = null)
    {
        if ($this->checkObjectId($objId)) {
            $dbRow = $this->senior_citizen_model->get_row(array("_id" => new ObjectId($objId), "service_data.appl_status" => "QS"));
            if ($dbRow) {
                $data = array(
                    "service_data.service_name" => $this->serviceName,
                    "dbrow" => $dbRow
                ...
                $this->load->view('includes/frontend/header');
                $this->load->view('seniorcitizen/seniorcitizenquery_view', $data);
                $this->load->view('includes/frontend/footer');
            } else {
    ...
    public function querysubmit()
    {
        ...
        $proofofRetirement = cifileupload("proof_of_retirement");
        $proof_of_retirement = $proofofRetirement["upload_status"] ? $proofofRetirement["uploaded_path"] : $this->input->post("proof_of_retirement_old");
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (count((array)$dbRow)) {
                ...
                if (strlen($proof_of_retirement)) {
                    $proof_of_retirement_type = (!empty($this->input->post("proof_of_retirement_type"))) ? $this->input->post("proof_of_retirement_type") : $dbRow->form_data->proof_of_retirement_type;
                    $proof_of_retirement = strlen($proof_of_retirement) ? base64_encode(file_get_contents(FCPATH . $proof_of_retirement)) : null;

                    $attachment_one = array(
                        "encl" =>  $proof_of_retirement,
                        "docType" => "application/pdf",
                        "enclFor" => "Proof of retirement",
                        "enclType" => $proof_of_retirement_type,
                        "id" => "65441672",
                        "doctypecode" => "7502",
                        "docRefId" => "7502",
                        "enclExtn" => "pdf"
    ...
    public function applicationpreview($objId = null)
    {
        ...
        if (count((array)$dbRow)) {
            ...
            $this->load->view('includes/frontend/header');
            //$this->load->view('nec/necertificatepreview_view',$data);
            $this->load->view('seniorcitizen/view_form_data', $data);
            $this->load->view('includes/frontend/footer');
        } else {
            $this->session->set_flashdata('error', 'No records found against object id : ' . $objId);
            redirect('spservices/seniorcitizen/scc/');
