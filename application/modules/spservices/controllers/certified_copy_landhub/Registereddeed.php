...
class Registereddeed extends Rtps {
    ...
    public function submit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (strlen($objId)) {
                ...
                // redirect('spservices/certified_copy_landhub/registereddeed/fileuploads/'.$objId);
                redirect('spservices/certified_copy_landhub/registereddeed/preview/' . $objId);
            } else {
                ...
                if ($insert) {
                    ...
                    redirect('spservices/certified_copy_landhub/registereddeed/preview/' . $objectId);
                    exit();
                } else {
    ...
    public function fileuploads($objId = null) {
        ...
        if (count((array) $dbRow)) {
            ...
            $this->load->view('includes/frontend/header');
            $this->load->view('certified_copy_landhub/registered_deep_enclosure', $data);
            $this->load->view('includes/frontend/footer');
        } else {
            $this->session->set_flashdata('error', 'No records found against object id : ' . $objId);
            redirect('spservices/certified_copy_landhub/registereddeed/');
        }//End of if else
    }//End of fileuploads()
    ...
    public function submit_to_backend($obj, $show_ack = false) {
        if ($obj) {
            ...
            //pre($postdata);
            $curl = curl_init($this->ePanjayeenURL . "cercpy/post_certicopy.php");
    ...
    public function finalsubmition() {
        die("This function is not available");
        if ($this->slug !== "user") {
            return false;
        }
        // this function is only for users
    ...
    public function preview($objId = null) {
        ...
        if (count((array) $dbRow)) {
            ...
            $this->load->view('includes/frontend/header');
            $this->load->view('certified_copy_landhub/registereddeed_view', $data);
            $this->load->view('includes/frontend/footer');
        } else {
            $this->session->set_flashdata('error', 'No records found against object id : ' . $objId);
            redirect('spservices/certified_copy_landhub/registereddeed/');
    ...
    public function query($objId = null) {
        ...
        if ($data["dbrow"]) {
            $this->load->view('includes/frontend/header');
            $this->load->view('certified_copy_landhub/query_perview', $data);
            $this->load->view('includes/frontend/footer');
        } else {
            $this->session->set_flashdata('error', 'No records found against object id : ' . $objId);
            redirect('spservices/certified_copy_landhub/registereddeed/');
    ...
    public function querysubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            redirect('spservices/certified_copy_landhub/registereddeed/query/' . $objId);
        }//End of if else        
    }//End of querysubmit()
