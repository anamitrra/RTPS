...
class Push_edistrict_no_records_data extends frontend
{
    ...
    public function retrive_edist_applications()
    {
        ...
        //         '$type' => "date",
        //         '$gte' => new MongoDB\BSON\UTCDateTime((time() - 80 * 60 * 60) * 1000),
        //     ],
        ...
        $command = array(
            'distinct' => 'edistrict_no_records',
            'key' => 'ssdg_ref_no',
            'query' => array(
                'landing_status' => array('$exists' => false, '$eq' => false),
                'repushed_application' => array('$exists' => false, '$eq' => false),
            )
        );
        ...
        for($i=0; $i<$total_appl; $i++) {
            ...
            if ($this->check_status($edistrict_ref_no)) {
                if ($this->reinitiate_application($service_id, $edistrict_ref_no)) {
                    ...
                    $data_to_update['landing_status'] = false;
                ...
                $data_to_update['landing_status'] = true;
            }
            $CI->mongo_db->set($data_to_update);
            $CI->mongo_db->where(array('ssdg_ref_no' => $edistrict_ref_no));
            $CI->mongo_db->update('edistrict_no_records');
        ...
        //                 '$type' => "date",
        //                 '$gte' => new MongoDB\BSON\UTCDateTime((time() - 3 * 60 * 60) * 1000),
        //                 '$lte' => new MongoDB\BSON\UTCDateTime((time() - 30 * 60) * 1000)
        //             ),
        //             "form_data.edistrict_ref_no" => array('$exists' => true, '$ne' => null, '$in' => $ssdgRefNos),
        //             "form_data.landing_status" => array('$exists' => false),
        ...
        // //     pre($document->form_data->edistrict_ref_no);
        // // }
    ...
    private function extractServiceID($refNo) {
        ...
        foreach ($parts as $part) {
            if (strpos($part, 'SCC') !== false || strpos($part, 'CASTE') !== false || strpos($part, 'NOK') !== false || strpos($part, 'INC') !== false || strpos($part, 'PDBR') !== false || strpos($part, 'PDDR') !== false | strpos($part, 'BAK') !== false || strpos($part, 'PRC') !== false) {
                $serviceID = $part;
                break;
            }
        }
