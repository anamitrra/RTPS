...
class Payment extends Rtps
{
    ...
    private function check_pfc_payment_status($DEPARTMENT_ID = null)
    {
        if ($DEPARTMENT_ID) {
            ...
            $ref = modules::load('spservices/employment_aadhaar_based/PaymentResponse');
    ...
    {
        if ($service_id === "EMP_A_REG") {
            $ref = modules::load('spservices/employment_aadhaar_based/Registration');
            $ref->post_data($obj_id);
        }
        if ($service_id === "EMP_A_RENEW") {
            $ref = modules::load('spservices/employment_aadhaar_based/Renewal');
            $ref->post_data($obj_id);
        }
        if ($service_id === "EMP_A_RE_REG") {
            $ref = modules::load('spservices/employment_aadhaar_based/Reregistration');
            $ref->post_data($obj_id);
        }
    }
    ...
    public function ctz_initiate($obj_id = null)
    {
        if ($obj_id) {
            ...
            if ($this->slug === "user") {
                ...
                $this->load->view('employment_aadhaar_based/ctz_payment', $data);
    ...
    public function initiate($obj_id = null)
    {
        if ($obj_id) {
            ...
            if ($this->slug !== "user") {
                ...
                $this->load->view('includes/frontend/header');
                $this->load->view('employment_aadhaar_based/kiosk_payment', $data);
                $this->load->view('includes/frontend/footer');
            } else {
                $this->payment_make($obj_id);
            }
    ...
    private function payment_make($obj_id = null)
    {
        ...
        if (count((array) $dbRow)) {
            ...
            if ($res) {
                $this->load->view('spservices/employment_aadhaar_based/payment', $data);
            } else {
