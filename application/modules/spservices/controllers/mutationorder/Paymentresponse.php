...
class Paymentresponse extends frontend {
    ...
    public function paymentmade() {
        ...
        $response = $_POST;
        $this->mutationorders_model->update_row(array('form_data.department_id' => $DEPARTMENT_ID), array("form_data.pfc_payment_status" => $_POST['STATUS'], "form_data.pfc_payment_response" => $response));
        if ($_POST['STATUS'] === 'Y') {
            //if (true) { //For app_test server
            if ($this->checkgrn($DEPARTMENT_ID, true) === $_POST['GRN']) {
                $this->post_data($DEPARTMENT_ID);
            } else {
                $this->mutationorders_model->update_row(array('form_data.department_id' => $DEPARTMENT_ID), array("form_data.pfc_payment_status" => "P", "form_data.pfc_payment_response.STATUS" => 'P'));
                $this->show_error();
            }//End of if else
        } else {
            $this->show_error();
    ...
    public function post_data($department_id = null) {
        ...
        $dbrow = $this->mutationorders_model->get_row(['form_data.department_id' => $department_id]);
        if (count((array) $dbrow)) {
            $obj_id = $dbrow->{'_id'}->{'$id'};           
            $appl_ref_no_temp = $dbrow->service_data->appl_ref_no;
            $appl_ref_no = "RTPS" . substr($appl_ref_no_temp, 4);     
            
            $postUrl = $this->config->item('dhar_api')."mutation_order/post_data_mutation_order.php";
            $newStatus = "PR";
            ...
            $mutation_doc = $dbrow->form_data->mutation_doc ? base64_encode(file_get_contents(FCPATH . $dbrow->form_data->mutation_doc)) : null;
            $revenue_receipt = $dbrow->form_data->revenue_receipt ? base64_encode(file_get_contents(FCPATH . $dbrow->form_data->revenue_receipt)) : null;
            $other_doc = $dbrow->form_data->other_doc ? base64_encode(file_get_contents(FCPATH . $dbrow->form_data->other_doc)) : null;
            
            $data = array(
                "spId" => array("applId" => $obj_id),
                "pdar_name" => $dbrow->form_data->pattadar_name,
                "loc_code" => $dbrow->form_data->patta->loc_code,
                "patta_no" => $dbrow->form_data->patta->patta_no,
                "application_ref_no" => $appl_ref_no,
                "dist_code" => $dbrow->form_data->district->district_code,
                "subdiv_code" => $appl_ref_no,
                "cir_code" => $dbrow->form_data->circle->circle_code,
                "mouza_pargona_code" => $dbrow->form_data->village->village_code,
                "lot_no" => $appl_ref_no,
                "vill_townprt_code" => $dbrow->form_data->village->village_code,
                "patta_type_code" => $dbrow->form_data->pattadar_name,
                "case_no" => $dbrow->form_data->case_no,
                "applicant_name" => $dbrow->form_data->applicant_name,
                "mobile" => $dbrow->form_data->mobile_number,
                "fathers_name" => $dbrow->form_data->fathers_name,
                "land_revenue_receipt" => array("encl" => $mutation_doc),
                "mutation_order_recpt" => array("encl" => $revenue_receipt),
                "other_docs" => array("encl" => $other_doc),
            );            
            ...
            curl_close($curl); 
            //echo $postUrl." : <br>"; echo '<pre>'; var_dump($data); echo '<pre>'; pre($response);            
            if(isset($error_msg)) {
                ...
                if ($res_status === "success") {
                    ...
                    $this->mutationorders_model->update($obj_id, $data_to_update);                    
                    //Sending submission SMS
                    $sms = array(
                        "mobile" => (int)$dbrow->form_data->mobile_number,
                        "applicant_name" => $dbrow->form_data->applicant_name,
                        "service_name" => 'Certified Copy of Mutation Order',
                    ...
                    redirect('spservices/mutationorder/registration/acknowledgement/' . $obj_id);
                } else {
                    return $this->output
    ...
    public function checkgrn($DEPARTMENT_ID = null, $check = false, $verify = false) { // TODO: need to check which are params to update
        $transaction_data = $this->mutationorders_model->get_row(array('form_data.department_id' => $DEPARTMENT_ID));
        if ($DEPARTMENT_ID) {
            ...
            if ($res) {
                ...
                //if($STATUS === "Y"){
                $this->mutationorders_model->update_row(
    ...
    public function cin_response() { //pre($_POST);
        if (!empty($_POST)) {
            if (!empty($_POST['DEPARTMENT_ID'])) {
                ...
                $this->mutationorders_model->update_row(array('form_data.department_id' => $DEPARTMENT_ID), array(
