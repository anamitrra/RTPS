...
class Incomecertificatepayment extends Rtps {
    ...
    public function application_submission($obj_id) {
        ...
        $ref = modules::load('spservices/incomecertificate/incomeresponse');
    ...
    public function initiate($obj_id = null) {
        ...
        if (count((array) $dbRow)) {
            ...
            $this->income_registration_model->update_where(['_id' => new ObjectId($obj_id)], $payData); //Update status            
            ...
            if (!empty($this->session->userdata('role'))) {
                ...
                $this->load->view('incomecertificate/incomepayment_view', $data);
    ...
    private function payment_make($obj_id = null) {
        $dbRow = $this->income_registration_model->get_by_doc_id($obj_id); //pre($dbRow);
        if (count((array) $dbRow)) {
                ...
                if(strlen($treasuryCode) && strlen($officeCode)) {
                    ...
                    if ($serviceRow) {
                        ...
                        $data['department_data'] = array(
                            ...
                            "SUB_SYSTEM" => "ARTPS-SP|" . base_url('spservices/incomecertificate/incomeresponse/paymentmade'),
                        );
