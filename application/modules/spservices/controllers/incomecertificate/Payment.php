...
class Payment extends Rtps {
    ...
    public function verify($obj_id=null){
      ...
      // die;
        if($obj_id){
          ...
          $application = $this->income_registration_model->get_row($filter);
    ...
    public function application_submission($obj_id,$service_id){
        if($service_id === "INC"){
          $ref=modules::load('spservices/incomecertificate/registration1');
    ...
    public function initiate($obj_id=null) {
      ...
      // die;
        ...
        if($obj_id){
          $dbRow = $this->income_registration_model->get_by_doc_id($obj_id);
          ...
          if (!empty($dbRow->form_data->payment_params) && !empty($dbRow->form_data->department_id)) {                
            ...
            $filter = array("_id" =>new ObjectId($obj_id));
            $this->income_registration_model->update_where($filter,array('service_data.appl_status'=>'payment_initiated'));
            // pre($filter);
            // die;
            ...
            $this->load->view('incomecertificate/kiosk_payment',$data);
    ...
    private function pfcpayment(){
        ...
        if($this->input->post('objid')){
         ...
         $transaction_data = $this->income_registration_model->get_row($filter);
       //  pre($transaction_data);
        ...
        $data['department_data']=array(
          ...
          "SUB_SYSTEM"=>"ARTPS-SP|".base_url('spservices/incomecertificate/payment_response/response'),
      ...
      public function update_pfc_payment_amount($data){
          ...
          $result=$this->income_registration_model->update_payment_status($appl_ref_no,$data_to_update);
    ...
    private function check_pfc_payment_status($DEPARTMENT_ID=null){
      ...
      if($DEPARTMENT_ID){
        ...
        $ref=modules::load('spservices/nextofkin/Payment_response');
