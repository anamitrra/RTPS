...
class Payment extends Rtps
{
    ...
    private function check_payment_status($DEPARTMENT_ID = null)
    {
        if ($DEPARTMENT_ID) {
            ...
            $ref = modules::load('spservices/prc/PaymentResponse');
    ...
    public function verify($obj_id = null)
    {
        ...
        if ($obj_id) {
            ...
            $application = $this->applications_model->get_row($filter);
            ...
            if (
                ...
                $res = $this->check_payment_status($application->form_data->department_id);
                if ($res) {
    ...
    {
        if ($service_id === "PRC") {
            $ref = modules::load('spservices/prc/Application');
    ...
    public function ctz_initiate($obj_id = null)
    {
        if ($obj_id) {
            ...
            if (isset($dbRow->form_data->payment_status) && $dbRow->form_data->payment_status == 'Y') {
                $this->application_submission($obj_id, "PRC");
            ...
            $filter = array("_id" => new ObjectId($obj_id));
            $this->applications_model->update_where($filter, array('service_data.appl_status' => 'payment_initiated'));
            ...
            if ($this->slug === "user") {
                ...
                $this->load->view('prc/ctz_payment', $data);
    ...
    public function initiate($obj_id = null)
    {
        if ($obj_id) {
            ...
            if ($this->slug !== "user") {
                ...
                $this->load->view('includes/frontend/header');
                $this->load->view('prc/kiosk_payment', $data);
                $this->load->view('includes/frontend/footer');
            } else {
                $this->payment_make($obj_id);
            }
    ...
    private function payment_make($obj_id = null)
    {
        ...
        if (count((array) $dbRow)) {
            ...
            if ($this->slug === "user") {
                ...
                //FIXME:::::CHNAGE BELOW ACCOUNT CONFIG WITH AMTRONS GRAS CONFIG
                 ...
                 $data['department_data'] = array(
                    ...
                    "SUB_SYSTEM" => "ARTPS-SP|" . base_url('spservices/prc/PaymentResponse/response'),
            ...
            //pre($res);
            if ($res) {
                $this->load->view('spservices/prc/payment', $data);
            } else {
    ...
    public function payment_update($data)
    {
        ...
        $result = $this->applications_model->update_payment($rtps_trans_id, $data_to_update);
        //pre ($result);
