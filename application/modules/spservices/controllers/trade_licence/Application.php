...
class Application extends Rtps
{
    ...
    private $serviceName = "APPLICATION FORM FOR TRADE LICENCE";
    private $serviceId = "NC-TL";

    public function __construct()
    {
        parent::__construct();
        $this->load->model('trade_licence/tradelicence_model');
        $this->load->model('trade_licence/circle_model');
        $this->load->model('trade_licence/state_model');
    ...
    public function index($objId = null)
    {
        ...
        $dbRow = $this->tradelicence_model->get_row(array('_id' => new ObjectId($objId)));

        //pre($dbRow);
        if ($dbRow) {
            $data["dbrow"] = $this->tradelicence_model->get_by_doc_id($objId);
        ...
        $this->load->view('trade_licence/application', $data);
    ...
    public function submit()
    {
       ...
       // pre(objId);
        ...
        $this->form_validation->set_rules('assistant_name', 'Assistant Name', 'trim|required|xss_clean|strip_tags');
        $this->form_validation->set_rules('market_place', 'Market Place', 'trim|required|xss_clean|strip_tags');
        $this->form_validation->set_rules('market_area', 'Market Area', 'trim|required|xss_clean|strip_tags');
    $this->form_validation->set_rules('business_nature', 'Nature of Business', 'trim|required|xss_clean|strip_tags');
        $this->form_validation->set_rules('annual_gross', 'Annual gross', 'trim|required|xss_clean|strip_tags');
        $this->form_validation->set_rules('place', 'Place', 'trim|required|xss_clean|strip_tags');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($formStatus === "QS") {
                if ($objId) {
                    $dbrow = $this->tradelicence_model->get_by_doc_id($objId);
            ...
            if ($formStatus === "QS") {
                ...
                while (1) {
                    ...
                    $rows = $this->tradelicence_model->get_row($filter);
            ...
            $form_data = [
                ...
                'market_area' => $this->input->post("market_area"),
                'business_nature' => $this->input->post("business_nature"),
                'annual_gross' => $this->input->post("annual_gross"),
            ...
            if (strlen($objId)) {
                $this->tradelicence_model->update_where(['_id' => new ObjectId($objId)], $inputs);
                $this->session->set_flashdata('success', 'Your application has been successfully updated');
                redirect('spservices/trade_licence/Application/preview/' . $objId);
            } else {
                $insert = $this->tradelicence_model->insert($inputs);

                if ($insert) {
                    $objectId = $insert['_id']->{'$id'};
                    $this->session->set_flashdata('success', 'Your application has been successfully submitted');
                    redirect('spservices/trade_licence/Application/preview/' . $objectId);
                    exit();
                } else {
    ...
    { //die($objId);
        ...
        if (count((array)$dbRow)) {
            ...
            $this->load->view('includes/frontend/header');
            $this->load->view('spservices/trade_licence/applicationpreview', $data);
            $this->load->view('includes/frontend/footer');
        } else {
            $this->session->set_flashdata('error', 'Records does not exist');
            redirect('spservices/trade_licence/Application');
        } //End of if else
    } //End of preview()
    ...
    { //die($objId);
        $dbRow = $this->tradepelicence_model->get_by_doc_id($objId);
        ...
        if (count((array)$dbRow)) {
            ...
            $this->load->view('includes/frontend/header');
            $this->load->view('spservices/trade_licence/applicationview', $data);
            $this->load->view('includes/frontend/footer');
        } else {
            $this->session->set_flashdata('error', 'Records does not exist');
            redirect('spservices/trade_licence/Application');
    ...
    public function finalsubmition($objId = null)
    {
        ...
        if ($objId) {
            ...
            $curl = curl_init("https://nchacartps.in/testsite/tl_postapplication.php");
            ...
            if ($response) {
                ...
                if ($response->ref->status === "success") {
                    ...
                    $this->tradelicence_model->update($objId, $data_to_update);
                    ...
                    if ($dbrow->service_data->appl_status === "QS") {
                        ...
                        redirect('spservices/trade_licence/Application/view/' . $objId);
                    } else {
                        //generate acknowlegement if success
                        redirect('spservices/trade_licence/Acknowledgement/acknowledgement/' . $objId);
                    }
                } else {
                    //redierct to application page if failure
                    $this->session->set_flashdata('error', 'Something went wrong please try again.');
                    redirect('spservices/trade_licence/Application/index/' . $objId);
                }
            }
        }
    }
    ...
    function getID($length)
    {
        ...
        while ($this->tradelicence_model->get_row(["rtps_trans_id" => $rtps_trans_id])) {
            $rtps_trans_id = $this->generateID($length);
        } //End of while()
    ...
    public function query($objId = null)
    {
        if ($this->checkObjectId($objId)) {
            ...
            $dbRow = $this->tradelicence_model->get_row($filter);
            //pre($dbRow);
            if ($dbRow) {
                //pre($dbRow);
                $data = array(
    ...
    public function track($objId = null)
    {
        ...
        if (count((array)$dbRow)) {
            ...
            $this->load->view('includes/frontend/header');
            $this->load->view('trade_licence/tptrack_view', $data);
            $this->load->view('includes/frontend/footer');
        } else {
            $this->session->set_flashdata('error', 'No records found against object id : ' . $objId);
            redirect('spservices/trade_licence/');
        } //End of if else
    } //End of track()
    ...
    {
        if ($obj) {
            $dbRow = $this->tradelicence_model->get_by_doc_id($obj);
            //pre($dbRow);
            if (count((array)$dbRow)) {
                ...
                */
                ...
                $curl = curl_init("https://nchacartps.in/taxation/testsite/update_tl.php");
                //$curl = curl_init($url); 
                curl_setopt($curl, CURLOPT_HTTPHEADER, array('Content-Type: application/json'));
                curl_setopt($curl, CURLOPT_POST, true);
                ...
                if (isset($error_msg)) {
                    ...
                    if ($response["ref"]["status"] === "success") {
                        ...
                        $processing_history[] = array(
                            "processed_by" => "Payment Query submitted by " . $dbRow->form_data->first_name,
                            "action_taken" => "Payment Query submitted",
                            "remarks" => "Payment Query submitted by " . $dbRow->form_data->first_name . " and <a href=\"" . base_url('spservices/trade_licence/Acknowledgement/payment_acknowledgement/' . $obj) . "\" target=\"_blank\"><b>Payment Acknowledment</b></a>",
                            "processing_time" => new UTCDateTime(strtotime(date('d-m-Y g:i a')) * 1000),
                        );
                        ...
                        $this->tradelicence_model->update_where(['_id' => new ObjectId($obj)], $data);

                        $this->session->set_flashdata('success', 'Your application has been successfully updated');
                        redirect('spservices/trade_licence/Acknowledgement/payment_acknowledgement/' . $obj);
                    } else {
