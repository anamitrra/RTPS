...
class Payment extends Rtps
{
    ...
    private $serviceName = "APPLICATION FORM FOR TRADING LICENCE";
    ...
    private function check_payment_status($DEPARTMENT_ID = null)
    {
        if ($DEPARTMENT_ID) {
            ...
            $min = $this->tradelicence_model->checkPFCPaymentIntitateTimeNew($DEPARTMENT_ID);
            ...
            $ref = modules::load('spservices/trade_licence/PaymentResponse');
    ...
    public function verify($obj_id = null)
    {
        ...
        if ($obj_id) {
            ...
            $application = $this->tradelicence_model->get_row($filter);
     ...
     public function ctz_initiate($obj_id = null)
    {
        if ($obj_id) {
            ...
            $filter = array("_id" => new ObjectId($obj_id));
            $this->tradelicence_model->update_where($filter, array('service_data.appl_status' => 'payment_initiated'));
            ...
            if ($this->slug === "user") {
                ...
                $this->load->view('trade_licence/ctz_payment', $data);
    ...
    public function initiate($obj_id = null)
    {
        if ($obj_id) {
            ...
            if ($this->slug !== "user") {
                ...
                $this->load->view('includes/frontend/header');
                $this->load->view('trade_licence/kiosk_payment', $data);
                $this->load->view('includes/frontend/footer');
            } else {
                $this->payment_make($obj_id);
            }
        ...
        if (count((array) $dbRow)) {
            ...
            if ($this->slug === "user") {
                 $data['department_data'] = array(
                    ...
                    "SUB_SYSTEM" => "ARTPS-SP|" . base_url('spservices/trade_licence/PaymentResponse/response'),
    ...
    public function payment_update($data)
    {
        ...
        $result = $this->tradelicence_model->update_payment($rtps_trans_id, $data_to_update);
        //pre ($result);
