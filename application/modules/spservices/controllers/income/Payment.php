...
class Payment extends Rtps
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('income/gras_config_model');
    ...
    public function application_submission($obj_id, $service_id)
    {
        if ($service_id === $this->serviceId) {
            $ref = modules::load('spservices/income/inc');
    ...
    private function check_pfc_payment_status($DEPARTMENT_ID = null)
    {
        if ($DEPARTMENT_ID) {
            ...
            $ref = modules::load('spservices/income/Payment_response');
    ...
    public function initiate($obj_id = null)
    {
        ...
        $dbRow = $this->income_model->get_by_doc_id($obj_id);
        if (count((array) $dbRow)) {
            ...
            $this->income_model->update_where(['_id' => new ObjectId($obj_id)], array('service_data.appl_status' => 'payment_initiated')); //Update status            
            ...
            if (!empty($this->session->userdata('role'))) {
                ...
                if ($this->slug === "CSC") {
                    $data['edistrict_amtron_charge'] = $this->config->item('edistrict_amtron_charge');
                } else {
                    $user = $this->users_model->get_by_id($this->session->userdata("userId")->{'$id'});
                    if ($user->type_of_kiosk == "eDistrict") {
                        $data['edistrict_amtron_charge'] = 0;
                    } else {
                        $data['edistrict_amtron_charge'] = $this->config->item('edistrict_amtron_charge');
                    }
                }
                ...
                $this->load->view('income/kiosk_payment', $data);
    ...
    private function payment_make($obj_id = null)
    {
        ...
        if (count((array) $dbRow)) {
            ...
            if ($this->slug === "user") {
                $application_charge = $this->config->item('edistrict_amtron_charge');
                $data['department_data'] = array(
                    "DEPT_CODE" => $dbRowGras->DEPT_CODE,
                    ...
                    "SUB_SYSTEM" => "ARTPS-SP|" . base_url('spservices/income/Payment_response/response'),
                    ...
                    "ACCOUNT1" => $dbRowGras->ACCOUNT1, //$user->account1, to be replaced with AMTRON Account code
                ...
                if ($this->slug === "CSC") {
                    ...
                    $data['department_data']['ACCOUNT1'] = $dbRowGras->ACCOUNT1;
    ...
    public function update_pfc_payment_amount($data, $application_charge)
    {
        ...
        $result = $this->income_model->update_payment_status($appl_ref_no, $data_to_update);
    ...
    public function verify($obj_id = null)
    {
        ...
        if ($obj_id) {
            ...
            $application = $this->income_model->get_row($filter);
