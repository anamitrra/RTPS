...
class Reinitiate_application_bakcl extends frontend
{
    ...
    public function post_data($edistrict_ref_no = null)
    {
        if (!empty($edistrict_ref_no)) {
            $filter = array(
                ...
                "service_data.service_id" => "BAKCL"
            );
            $dbRow = $this->bakijai_model->get_row($filter);
            if (!empty($dbRow)) {
                $obj = $dbRow->{'_id'}->{'$id'};
                ...
                if (!empty($dbRow->form_data->soft_copy)) {
                    ...
                    if (isset($error_msg)) {
                        ...
                        if (isset($response->ref->status) && $response->ref->status === "success") {
                            ...
                            return array(
                                "message" => "Invalid response received from the eDistrict POST API!",
                                "status" => false
                            );
                        }
                    }else{
