        ...
        <div class="card shadow">
            ...
            <div class="card-body">
                <table class="table  table-sm" id="application_list_table">
                    <tr>
                        <td>
                            <p style="font-size:16px"><b>RTPS Ref. No. : <span class="text-danger"><?php echo $dbrow->form_data->rtps_trans_id; ?></span></b></p>
                        </td>
                        <td>
                ...
                <?php if (isset($dbrow->service_data->appl_status) && $dbrow->service_data->appl_status == "QS") { ?>
                    <p style="font-size:18px;"><label><i class="fa fa-hand-o-right" aria-hidden="true"></i><span class="text-danger"> Waiting for applicant response. Please <a href="<?= base_url('spservices/prc/Application/query/' . $dbrow->_id->{'$id'}) ?>" class="btn btn-info btn-sm">click here</a> to proceed.</span></label></p>
                <?php } elseif (isset($dbrow->service_data->appl_status) && $dbrow->service_data->appl_status == "D") { ?>
                    <a class="btn btn-success btn-sm mbtn text-center" target="_blank" href="<?= base_url('spservices/prc/Application/download_certificate/' . $dbrow->_id->{'$id'}) ?>"><i class="fa fa-download" aria-hidden="true"></i> DOWNLOAD CERTIFICATE</a>
                <?php }
                ?>
