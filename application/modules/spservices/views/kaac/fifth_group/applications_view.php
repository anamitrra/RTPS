...
$applications = !empty($karbi_farmers) ? $karbi_farmers : array();
...
<div class="container">
  <div class="row">
    <div class="col-sm-12 mx-auto">
      <div class="card my-4">
        <div class="card-body">
          <h4>Application Form for Farmer's Certificate <a
                            href="<?= base_url('spservices/kaac_farmer/registration/index') ?>"
            ...
            <table class="table">
              ...
              <tbody>
                ...
                ?>
                    <tr>
                      <td><?= ($key + 1) ?></td>
                      <td><a href="<?= base_url("spservices/kaac_farmer/registration/applicationpreview/") . $obj_id ?>" target="_blank"><?= $value->service_data->appl_ref_no ?></a></td>
                      ...
                      <td>
                        <?php if ($value->service_data->appl_status == "DRAFT") { ?>
                          <a class="btn btn-primary btn-sm mbtn" href="<?= base_url("spservices/kaac_farmer/registration/index/") . $obj_id ?>">Complete Your Application</a>
                        <?php } elseif ($value->service_data->appl_status == "submitted") { ?>
                          <a class="btn btn-success btn-sm mbtn" href="<?= base_url("spservices/kaac_farmer/registration/acknowledgement/") . $obj_id ?>">Acknowledgement</a>
                        <?php } elseif ($value->service_data->appl_status == "QS") {
                        ?>
                          <a class="btn btn-success btn-sm mbtn" href="<?= base_url("spservices/kaac_farmer/registration/acknowledgement/") . $obj_id ?>">Acknowledgement</a>
                          <a class="btn btn-warning btn-sm mbtn" href="<?= base_url('spservices/kaac_farmer/registration/queryform/' . $obj_id) ?>">Reply to query</a>
                        <?php
                          ...
                          ?>
                              <form method="post" name="getGRN" id="getGRN" action="<?= $this->config->item('egras_grn_cin_url') ?>">
                                ...
                                <input type="hidden" id="SUB_SYSTEM" name="SUB_SYSTEM" value="ARTPS-SP|<?= base_url('spservices/kaac_farmer/payment_query_response/cin_response') ?>" />
                            ...
                            ?>
                              <a class="btn btn-warning btn-sm mbtn" href="<?= base_url('spservices/kaac_farmer/registration/querypaymentsubmit/' . $obj_id . '/pp') ?>">Verify Query Payment</a>
                            <?php
                            } else {  ?>
                            
                              <a class="btn btn-warning btn-sm mbtn" href="<?= base_url('spservices/kaac_farmer/registration/query_payment_break_down/' . $obj_id) ?>">Make Query Payment</a>
                          <?php }
                          } ?>
                        ...
                        <?php } elseif ($value->service_data->appl_status == "payment_initiated") { ?>
                          ...
                          ?>
                            <form method="post" name="getGRN" id="getGRN" action="<?= $this->config->item('egras_grn_cin_url') ?>">
                              ...
                              <input type="hidden" id="ACTION_CODE" name="ACTION_CODE" value="GETCIN" readonly />
                              <input type="hidden" id="SUB_SYSTEM" name="SUB_SYSTEM" value="ARTPS-SP|<?= base_url('spservices/kaac_farmer/payment_response/cin_response') ?>" />
                          ...
                          ?>
                            <a class="btn btn-warning btn-sm mbtn" href="<?= base_url('spservices/kaac_farmer/registration/submit_to_backend/' . $obj_id) ?>">Verify Payment</a>
                          <?php
                          } else {  ?>
                            <a class="btn btn-warning btn-sm mbtn" href="<?= base_url('spservices/kaac_farmer/payment/verify/' . $obj_id) ?>">Make/Verify Payment</a>
                          <?php }
                          ?>
                        <?php } elseif ($value->service_data->appl_status == "S" || $value->service_data->appl_status == "QA" || $value->service_data->appl_status == "PR" || $value->service_data->appl_status == "R" || $value->service_data->appl_status == "F") { ?>
                          <a class="btn btn-success btn-sm mbtn" href="<?= base_url("spservices/kaac_farmer/registration/acknowledgement/") . $obj_id ?>" target="_blank">Acknowledgement</a>
                          <?php if ((isset($value->form_data->frs_request->bpPaymentTypeStatus) && !empty($value->form_data->frs_request->bpPaymentTypeStatus) == true) && (isset($value->form_data->frs_request->ppPaymentTypeStatus) && !empty($value->form_data->frs_request->ppPaymentTypeStatus) == true)) { ?>
                            <a class="btn btn-success btn-sm mbtn" href="<?= base_url('spservices/kaac_farmer/registration/payment_acknowledgement/' . $obj_id) ?>">Payment Acknowledgement</a>
                          <?php } ?>
                        <?php } elseif ($value->service_data->appl_status == "D") { ?>
                          ...
                          <?php if ((isset($value->form_data->app_record_type)) && (!empty($value->form_data->app_record_type) != "reapply_done") && $value->service_data->appl_status == "D") { ?>
                            <a class="btn btn-success btn-sm mbtn" href="<?= base_url('spservices/kaac_farmer/registration/cancel_form/' . $obj_id) ?>">Cancel Permit</a>
                          <?php } ?>
                        <?php }
                        ?>

                        <a href="<?= base_url('spservices/kaac_farmer/registration/track/' . $obj_id) ?>" class="btn btn-secondary btn-sm mbtn">Track</a>
                      </td>
