        ...
        <div class="card shadow-sm" id="printDiv">
            ...
            <div class="card-body" style="padding:5px">
                ...
                <fieldset class="border border-success table-responsive" style="margin-top:10px">
                    ...
                    <table class="table table-borderless" style="margin-top:0px; margin-bottom:0px;">
                        <tbody style="border-top: none !important">
                            <tr>
                            <td style="width:50%">Name of the assistants employed or intended to be employed<strong> : <?=$assistant_name?> </strong> </td>
                                <td>Nature of Commodities<strong> :<?=$commodities?> </strong> </td>
                            </tr>
                            <tr>
                                <td>Market Place<strong> : <?=$market_place?></strong> </td>
                                
                                <td>Marketing Area<strong> : <?=$market_area?></strong> </td>
                            </tr>
                            <tr>
                                <td>Nature of Business<strong> : <?=$business_nature?></strong> </td>
                                
                                <td>Annual gross outturn will be about Rs.<strong> : <?=$annual_gross?></strong> </td>
                            </tr>
                        </tbody>
                    </table>
                </fieldset>
            ...
            <div class="card-footer text-center no-print">
                <?php if($form_status === 'DRAFT') { ?>    
                    <a href="<?=base_url('spservices/trade_licence/Application/index/'. $obj_id)?>" class="btn btn-primary">
                ...
                <?php if($payment_status !== 'PAYMENT_COMPLETED'){
                    if($form_status === 'QS'){ ?>
                        <!--<button class="btn btn-warning frmbtn"  type="button">
                            <i class="fa fa-angle-double-right"></i> Submit
                        </button>
                         -->
                        <a href="<?=base_url('spservices/trade_licence/Application/finalsubmition/'. $obj_id)?>" class="btn btn-warning" onclick='this.disabled = true;'>
                        <i class="fa fa-pencil"></i> Submit
                    </a>
                     <?php } else if($user_type == 'user'){ ?>
                        <a href="<?=base_url('spservices/trade_licence/Payment/ctz_initiate/'. $obj_id)?>" class="btn btn-warning">
                        <i class="fa fa-angle-double-right"></i> Make Payment
                    </a>
                    <?php } else{ ?>
                        <!--
                        <button class="btn btn-warning frmbtn" id="cscpfc_pay" type="button">
                            <i class="fa fa-angle-double-right"></i> Make Payment
                        </button>-->
                        <a href="<?=base_url('spservices/trade_licence/Payment/initiate/'. $obj_id)?>" class="btn btn-warning">
                        <i class="fa fa-angle-double-right"></i> Make Payment
                    </a>
                    <?php }//End of if else
                }//End of if ?>
