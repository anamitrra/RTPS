    ...
    $(document).ready(function() {
        ...
        function getOtp(aadhaar, name) {
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "<?= base_url('spservices/employment_aadhaar_based/registration/otpsend') ?>",
                data: {
        ...
        $(document).on("click", ".verify_otp", function() {
            ...
            if (/^\d{6}$/.test(otpNo)) {
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "<?= base_url('spservices/employment_aadhaar_based/registration/otpverify') ?>",
                    data: {
                    ...
                    success: function(res) { //alert(JSON.stringify(res));
                        ...
                        if (Object.values(res.ret)[0] === 'y') {
                            ...
                            var msg_str = '';
                            // alert(res.msg);
                            if (typeof res.msg === 'string' || res.msg instanceof String) {
                                msg_str = res.msg;
                            }
                            else {
                                msg_str = 'Aadhaar verification failed. Please try after sometime.';
                            }

                            alertMsg('error', msg_str);
                            $('#verify_modal').modal("show");
