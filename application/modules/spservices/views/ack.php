          ...
          <div class="card-body">
            ...
            <div class="row">
              <p style="text-align:justify; line-height: 24px">
                ...
                <br />
                <br />
                Your application for <strong> <?= isset($service_row->service_name) ? $service_row->service_name : '' ?></strong> has been submitted successfully on <strong><?= isset($response->submission_date) ? format_mongo_date($response->submission_date) : '' ?></strong> and
                ...
                If the application has been accepted by the <strong><?= isset($service_row->department_name) ? $service_row->department_name : '' ?></strong> the service shall be provided within
                stipulated timeline . You may raise an appeal at http://rtps.assam.gov.in/ if the service is not delivered within the <strong><?= isset($service_row->stipulated_timeline) ? $service_row->stipulated_timeline . ' days' : '' ?></strong> days.
            ...
            <?php if (!empty($response->amount) || (!empty($response->pfc_payment_status) && $response->pfc_payment_status === "Y")) { ?>
              ...
              <div class="row">
                <table>
                  ...
                  <?php if (strpos($response->rtps_trans_id, "ACMRREGAD")) : ?>
                    <tr>
                      <td><b>GST:</b></td>
                      <td>900.00</td>
                    </tr>
                  ...
                  <?php if (strpos($response->rtps_trans_id, "CPCME")) : ?>
                    <tr>
                      <td><b>GST:</b></td>
                      <td>360.00</td>
                    </tr>
          ...
          <div class="card-body">
            ...
            <div class="row">
              <p style="text-align:justify; line-height: 24px">
                ...
                <strong> <?= isset($service_row->service_name) ? $service_row->service_name : '' ?></strong> বাবে দাখিল কৰা আপোনাৰ আবেদনপত্রখন <strong><?= isset($response->submission_date) ? format_mongo_date($response->submission_date) : '' ?></strong> তাৰিখে সফলতাৰে সম্পূর্ণ হৈছে আৰু আপোনাৰ স্বীকৃতি সংখ্যা হৈছে, <strong><?= isset($response->rtps_trans_id) ? $response->rtps_trans_id : '' ?></strong> । ভৱিষ্যতে আবেদন সম্পর্কীয় যিকোনো যোগাযোগৰ বাবে আৰু আবেদনৰ স্থিতি নিৰীক্ষণ কৰিবলৈ, অনুগ্ৰহ কৰি এই স্বীকৃতি নম্বৰটো ব্যৱহাৰ কৰক। যদি আবেদনপত্ৰখন <strong><?= isset($service_row->department_name) ? $service_row->department_name : '' ?></strong> বিভাগৰ দ্বাৰা গ্ৰহণ কৰা হৈছে, তেন্তে <strong><?= isset($service_row->stipulated_timeline) ? $service_row->stipulated_timeline : '' ?></strong> দিনৰ ভিতৰত সেৱা প্ৰদান কৰিব লাগিব। যদি <strong><?= isset($service_row->stipulated_timeline) ? $service_row->stipulated_timeline : '' ?></strong> দিনৰ ভিতৰত সেৱা আগবঢ়োৱা নহয়, তেনেহলে আবেদনকাৰীয়ে http://rtps.assam.gov.in/ ত আবেদন কৰিব পাৰে। আপুনি আমাক ১৮০০-৩৪৫-৩৫৭৪ নম্বৰত প্ৰতি সোমবাৰৰ পৰা শনিবাৰলৈ, পুৱা ৮ বজাৰ পৰা নিশা ৮ বজালৈ ফোন কৰিব পাৰে অথবা
...
function calculateTotal($response)
{
  ...
  if (strpos($response->rtps_trans_id, "CPCME") !== false) {
    $total += 360; // Add 360 to the total if "CPCME" is found in rtps_trans_id
  }
