<?php 
$appl_ref_no = isset($dbrow->rtps_trans_id) ? $dbrow->rtps_trans_id: '';
$submission_date = isset($dbrow->submission_date) ? format_mongo_date($dbrow->submission_date) : '';
$applicant_name = '';
...
$amount = isset($dbrow->amount) ? $dbrow->amount : 0;
$pfc_payment_status = isset($dbrow->pfc_payment_status) ? $dbrow->pfc_payment_status : '';
$rtps_convenience_fee = $dbrow->rtps_convenience_fee;
$service_charge = isset($dbrow->service_charge) ? $dbrow->service_charge : 0;
$no_printing_page = isset($dbrow->no_printing_page)?$dbrow->no_printing_page:0;
$printing_charge_per_page = isset($dbrow->printing_charge_per_page)?$dbrow->printing_charge_per_page:0;
$no_scanning_page = isset($dbrow->no_scanning_page)?$dbrow->no_scanning_page:0;
$scanning_charge_per_page = isset($dbrow->scanning_charge_per_page)?$dbrow->scanning_charge_per_page:0;
?>
<style type="text/css">
    .print {
        visibility:visible;
...
function calculateTotal($dbrow){
  ...
  if (!empty($dbrow->pfc_payment_status) && $dbrow->pfc_payment_status === "Y"){
    ...
    if(!empty($dbrow->rtps_convenience_fee)){
        $total += $dbrow->rtps_convenience_fee;
    }
