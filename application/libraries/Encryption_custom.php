...
class Encryption_custom
{
    ...
    public function decrypt($encryptedString, $key)
    {
        ...
        try {
            $salt = hex2bin(isset($json["salt"] ) ? $json["salt"] : "");
            $iv = hex2bin(isset($json["iv"])?$json["iv"] : "");
        } catch (Exception $e) {
            return null;
        }

        $cipherText = base64_decode(isset($json['ciphertext'])?$json['ciphertext']:"");

        $iterations = intval(abs(isset($json['iterations'])?$json['iterations']:""));
        if ($iterations <= 0) {
            $iterations = 999;
        }
