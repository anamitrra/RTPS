 ...
 * @copyright Copyright (c) 2020 Prasenjit Das | 9401250708
 */
class Mongo_db
{
	private $CI;
	...
	private $likeas_master	= array();
	private $likeas	= array();
	...
	public function get_data_like($filter, $collection)
	{
		if (empty($collection)) {
	...
	public function mongo_count($collection = NULL)
	{
		if (empty($collection)) {
			show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
		}
		try {
			...
			$cmd["query"] = $query;
			$cursor = $this->db->executeCommand($this->database, new MongoDB\Driver\Command($cmd));
			$result = current($cursor->toArray());

			// Older server versions may return a float
			if (!isset($result->n) || !(is_integer($result->n) || is_float($result->n))) {
				show_error('count command did not return a numeric "n" value');
			}
	...
	public function mongo_like_count($filter, $collection = NULL)
	{
		if (empty($collection)) {
			show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
		}
	 ...
	 */
	public function like($field = "", $value = "", $flags = "i", $enable_start_wildcard = FALSE, $enable_end_wildcard = TRUE)
	{
		if (empty($field)) {
		...
		$regex = "/$value/$flags";
		$this->likeas[$field] = ['$regex' => $value, '$options' => $flags];
	 ...
	 */
	public function get_like($collection = "")
	{
		if (empty($collection)) {
			show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
		}
		try {
			...
			foreach ($this->likeas as $key => $val) {
				array_push($this->likeas_master, array($key => $val));
			}
			$temp = array();
			$temp['$or'] = $this->likeas_master;
			$query = new MongoDB\Driver\Query($this->likeas, $options);
			print_r($query);
			$cursor = $this->db->executeQuery($this->database . "." . $collection, $query, $read_preference);
			// Clear
			$this->_clear();
	 ...
	 */
	public function order_by($fields = array(), $value = NULL)
	{
		if (is_array($fields)) {
	...
	public function getDateTime($obj = NULL)
	{
	...
	/*********Timestamp is already in Asia/Kolkata zone*********/
	public function getDateTimeWithoutTZ($obj = NULL)
	{
		$datetime = $obj->toDateTime();
		return $datetime->format("d-m-Y H:i:s");
	}
	 ...
	 */
	public function limit($x = 99999, $offset = 0)
	{
		$this->offset($offset);
